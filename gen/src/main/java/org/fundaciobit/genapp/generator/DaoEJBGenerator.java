...
public class DaoEJBGenerator {
  ...
  public static void generaMissatges(String projectName, TableInfo[] tables, File dstEjbDir, String[] idiomes) throws IOException,
      Exception {
    ...
    for (String lang : idiomes) {
      ...
      if (startMark == -1 || endMark == -1) {
        throw new Exception("El fitxer de misstages " + missatges
      ...
      */
      ...
      for (TableInfo table : tables) {
        ...
        for (FieldInfo field : table.getFields()) {          
          str.append(model + "." + field.javaName + "=" + field.getLabels().get(lang).replace('\'', '´') + "\n");
        }
  ...
  public static SourceFile generateCodeForManagerLocal(Project projecte, String tableNameJava, 
      String jpaPackage,String ejbPackage, BasicPackages packages,  BeanCode beanCode) {
  ...
  public static SourceFile generateCodeForManagerEJB(Project projecte,
      String tableNameJava, String jpaPackage, String ejbPackage, 
      BasicPackages basicPackages, BeanCode beanCode) {
    ...
    manager.append("@Stateless(name = \"" + managerFileName + "\")\n");
    /*
    ...
    manager.append("\t@RolesAllowed({\"" + prefix + "_ADMIN\","
        + "\"" + prefix + "_USER\"})\n");    
    ...
    manager.append("\tpublic " + tableNameJava + " create(" + tableNameJava + " instance) throws I18NException {\n");
