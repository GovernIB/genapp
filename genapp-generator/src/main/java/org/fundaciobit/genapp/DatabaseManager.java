...
public final class DatabaseManager {
    ...
    public static final String sJDBCTypes[][] = {
            { "MySQL", "com.mysql.jdbc.Driver", "jdbc:mysql://devlinux:3306/vigitec" },
            { "HypersonicSQL Standalone", "org.hsqldb.jdbcDriver", "jdbc:hsqldb:file:<<PATH>>" },
            ...
            { "HypersonicSQL Server", "org.hsql.jdbcDriver", "jdbc:HypersonicSQL:hsql://localhost" },
            { "HypersonicSQL WebServer", "org.hsql.jdbcDriver", "jdbc:HypersonicSQL:http://localhost" },
            ...
            { "Cloudscape RMI", "RmiJdbc.RJDriver", "jdbc:rmi://localhost:1099/jdbc:cloudscape:test;create=true" },
            { "InstantDb", "jdbc.idbDriver", "jdbc:idb:sample.prp" },
            { "PointBase", "com.pointbase.jdbc.jdbcUniversalDriver", "jdbc:pointbase://localhost/sample" }, // PUBLIC / public
    };
    ...
    public TableInfo[] getTablesOfDataBase(String schema, String[] idiomes) throws Exception, SQLException {
        ...
        this.databaseInfo.setDatabaseProductName(dMeta.getDatabaseProductName());
        ...
        if (schema == null || schema.trim().length() == 0) {
            ...
            schemaName = (String) JOptionPane.showInputDialog(null, "Selecciona l'esquema:", "Selecciona l'esquema",
    ...
    public Fields getFieldsInfoOfTable(String schema, String tableName, String[] idiomes) throws Exception {
        ...
        Map<String, List<ForeignKey>> foreignKeys = getForeignKeys(tableName, conn, dMeta);
        ...
        while (resultSet.next()) {
            ...
            // JAVA TYPE
            {
                ...
                if (newRow.isPrimaryKey() || newRow.isNotNullable || Boolean.class.equals(newRow.getJavaType())) {
                    try {
                        Class<?> primitive = SQL2Java.javaClass2PrimitiveType(newRow.getJavaType());
                ...
                try {
                    ...
                    log.error("No s'ha pogut obtenir el valor per defecte de " + tableName + "::" + columnName + ": "
                            + e.getMessage());
                }

                log.info("\n\n\n VALOR PER DEFECTE => " + newRow.getDefaultValue() + "\n"
                        + "DATABASE (product_name) => " + dMeta.getDatabaseProductName() + "\n\n\n");
            }
            ...
            newRow.setAutoIncrement(autoInc == null ? false : autoInc.booleanValue());

            newRow.webFieldInfo = WebFieldInfo.getDefaultWebTypeFromFieldInfo(tableName, newRow);
    ...
    private static Map<String, String> getIndexFields(String schemaName, String tableName, DatabaseMetaData meta,
            Set<String> ignoreIndexes) throws SQLException {
    ...
    private static Map<String, List<String>> getUniqueFields(String schemaName, String tableName, DatabaseMetaData meta)
            throws SQLException {
     ...
     * 
     */
    private Map<String, List<ForeignKey>> getForeignKeys(String tableName, Connection conn, DatabaseMetaData dMeta)
            throws SQLException {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            while (rs.next()) {
                ...
                log.info("====[" + fkName + "] " + methodName + " FOREIGN KEY[" + columnNameOrig + "] -> "
                        + tableNameDst + "[" + columnNameDst + "]");
