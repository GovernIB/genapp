    ...
    */
    ...
    public boolean initPanel() {
        ...
        switch (SharedData.project) {
            ...
            case UPDATE:
                ...
                projectName.setText(proj.getProjectName());
                url.setText(proj.getDataBaseInfo().getUrl());
                driver.setText(proj.getDataBaseInfo().getDriver());
                usr.setText(proj.getDataBaseInfo().getUsr());
    ...
    public boolean next() {
        ...
        if (nametxt.trim().equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'Project Name' esta buit", "error",
                    JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        SharedData.data.setProjectName(nametxt.trim());

        String ms = modulSeguretat.getText();
        if (ms.trim().equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'Modul de Seguretat' esta buit", "Error",
                    JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        SharedData.data.setModulSeguretat(ms);

        String packtxt = this.pack.getText();
        if (packtxt.trim().equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'Package' esta buit", "error", JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        ...
        if (prefixtxt.trim().equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'Prefix' esta buit", "error", JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        ...
        if (idiomestxt.trim().equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'Idiomes' esta buit", "error", JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        String[] langs = idiomestxt.replace(" ", "").split(",");
        if (langs.length == 0) {
            JOptionPane.showMessageDialog(new Frame(), "'Idiomes' esta mal escrit", "error",
                    JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        ...
        if (urltxt.equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'url' esta buit", "error", JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }

        String drivertxt = (String) this.driver.getText();
        if (drivertxt.equals("")) {
            JOptionPane.showMessageDialog(new Frame(), "'driver' esta buit", "error", JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }

        try {
            SharedData.data.setDataBaseInfo(new DataBaseInfo(drivertxt, urltxt, usr.getText(), pwd.getText()));
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "error connectatnt amb bd\n" + e.getMessage(), "error",
                    JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
        ...
        try {
            ...
            manager = new DatabaseManager(SharedData.data.getDataBaseInfo());
            ...
            switch (SharedData.project) {
                ...
                case UPDATE: {
                    TableInfo[] ti = SharedData.data.getTables();
                    ...
                    TableInfo tmp[] = manager.getTablesOfDataBase(SharedData.data.getSchema(),
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Error Seleccionant taules de la BD\n" + e.getMessage(), "error",
                    JOptionPane.INFORMATION_MESSAGE);
            return (false);
        }
            ...
            case NEW:
                try {
                    ...
                    for (int i = 0; i < tablesFromDB.length; i++) {
                        ...
                        Fields fields = manager.getFieldsInfoOfTable(SharedData.data.getSchema(), tablesFromDB[i],
                                idiomes);
    ...
    public TableInfo[] mergeTables(DatabaseManager manager, TableInfo[] fromFile, String[] fromDB, String[] idiomes)
            throws Exception {
            ...
            if (updateItems.contains(nameBBDD)) {
                ...
                for (int j = 0; j < fieldsFromFile.length; j++) {
                    ...
                    if (updatedFields.contains(fieldsFromFile[j])) {
                        log.info("    " + j + ".-Update Field[" + fieldsFromFile[j].sqlName);
                ...
                TableInfo tableInfo = new TableInfo(nameBBDD, nameJava, shortName, labels, labelsPlural,
                        mergedFields.toArray(new FieldInfo[mergedFields.size()]), fromFile[i].generate);
        ...
        log.info(">>>>>>>>>>>>>>>>>  NEW TABLES (" + newItems.size() + ") <<<<<<<<<<<<<<<<<");
