...
/**
 * Title: GenApp v2 2020 (Rapit Entity Bean 2010) Description: Copyright:
 * Copyright (c) 2014 Company: XmasSoft
 * 
 * @author anadal
 */
public class DaoJPAGenerator {

    public static SourceFile[] generateHibernateUtils(String hibernatePackage, String hibernateName,
            String hibernateFileName) {
    ...
    public static SourceFile[] generateCustomHibernatePersistenceProvider(String hibernatePackage, String hibernateName,
            String hibernateFileName, Project project) {
        ...
        String code2 = "package " + hibernatePackage + ";\n" + "\n" + "import java.util.HashMap;\n"
                ...
                + "import org.slf4j.LoggerFactory;\n" + "\n" + "import " + project.getPackageName()
                + ".commons.utils.Configuracio;\n" + "/**\n" + " * ========= FITXER AUTOGENERAT - NO MODIFICAR !!!!!\n"
                + " * @author jagarcia\n" + " */\n" +
                // XYZ ZZZ ZZZ
                ...
                + project.getPackageName() + ".hibernate\")) {\n"
                + "	        		projecteProperties.put(kStr.replace(\"" + project.getPackageName()
    ...
    public static SourceFile generateJPABeanCode(Project project, TableInfo table, String entityPackage,
            String jpaPackage, String hibernatePackage, BeanCode beanGenCode) throws Exception {
        ...
        for (int r = 0; r < fields.length; r++) {
            ...
            if (!field.isTransientField()) {
                if (field.getIndex() != null) {
                    ...
                    strIndexes.append("\n        @Index(name=\"" + field.getIndex() + "\", columnList = \""
                            + field.sqlName + "\")");
                }
            }
        }
        ...
        if (!table.isTranslationMapEntity()) {
            ...
            if (uniques != null && uniques.length != 0) {
                ...
                for (int i = 0; i < uniques.length; i++) {
                    ...
                    beanCode.append(
                            " @UniqueConstraint(" + "name=\"" + uniques[i].getName() + "\"," + " columnNames={");
        ...
        beanCode.append("public class " + codeBeanFileName + " implements " + tableNameJava + " {\n\n");
        // Llista d'Atributs
...
//          }
                ...
                if (field.isAutoIncrement() || defValueClean == null || defValueClean.isEmpty()
                    ...
                    // llevar coses com 'ca'::character varying
                    int i2punts = defValueClean.lastIndexOf("::");

                    if (i2punts != -1) {
                        defValueClean = defValueClean.substring(0, i2punts);
                    }

                    defValueClean = defValueClean.trim();

                    if (defValueClean.startsWith("'") && defValueClean.endsWith("'")) {
                        defValueClean = defValueClean.replace("\"", "\\\"");
                    }

                    if (isBoolean(field)) {
                        if (defValueClean.charAt(0) == '0') {
                            defValueClean = "false";
                ...
                /*
                 * ForeignKey[] fk = field.getForeignKeysByType(ForeignKey.IMPORTED); if (fk !=
                 * null && fk.length > 0) {
        ...
        if (!table.isTranslationMapEntity()) {
            beanCode.append(generateImportsExports(project, table, fields, imports, jpaPackage, CodeType.JPA,
                    importForeignKeys, exportForeignKeys));
        }
        ...
        if (table.isFileMappingEntity()) {
            ...
            beanCode.append("      this.encryptedFileID = " + hibernatePackage
                    + ".HibernateFileUtil.encryptFileID(fitxerID);\n");
            beanCode.append("    }\n");
            beanCode.append("  }\n");
            beanCode.append("\n");
        }
        ...
        if (!table.isTranslationMapEntity()) {
            ...
            beanCode.append(generateCopyJPACode(table, importForeignKeys, exportForeignKeys, fields, codeBeanFileName,
                    "copyJPA", fileFields, fileTable, "JPA"));
        }
        ...
        return new SourceFile(codeBeanFileName + ".java", header.toString() + beanCode.toString(), true);

    }

    protected static boolean isBoolean(FieldInfo field) {
        return field.getJavaType().equals(Boolean.class) || field.getJavaType().equals(boolean.class);
    }
    ...
    public static String generateCopyJPACode(TableInfo table, HashMap<String, String> importForeignKeys,
            HashMap<String, String> exportForeignKeys, FieldInfo[] fields, String codeBeanFileName,
            String copyMethodName, List<FieldInfo> fileFields, TableInfo fileTable, String fileSuffix)
            throws Exception {
        ...
        beanCode.append("\n");
        beanCode.append(
                "  public static " + codeBeanFileName + " " + copyMethodName + "(" + codeBeanFileName + " __jpa) {\n");
        beanCode.append("    return " + copyMethodName + "(__jpa,new java.util.HashMap<" + entityBase + "," + entityBase
                + ">(), null);\n");
        beanCode.append("  }\n\n");

        /*
         * static Set<PeticioDeFirmaJPA> copyJPA(Set<PeticioDeFirmaJPA> __jpaSet,
         * java.util.Map<org.fundaciobit.genapp.common.IGenAppEntity,org.fundaciobit.
         * genapp.common.IGenAppEntity> __alreadyCopied) { if (__jpaSet == null) {
         * return null; } Set<PeticioDeFirmaJPA> __tmp = (Set<PeticioDeFirmaJPA>)
         * __alreadyCopied.get(__jpaSet); if (__tmp != null) { return __tmp; }; __tmp =
         * new HashSet<PeticioDeFirmaJPA>(); __alreadyCopied.put(__jpaSet, __tmp);
         */

        beanCode.append("  static java.util.Set<" + codeBeanFileName + "> " + copyMethodName + "(java.util.Set<"
                + codeBeanFileName + "> __jpaSet,\n" + "    java.util.Map<" + entityBase + "," + entityBase
                + "> __alreadyCopied, String origenJPA) {\n");
        beanCode.append("    if (__jpaSet == null) { return null; }\n");

        // beanCode.append(" @SuppressWarnings(\"unchecked\")\n");

        beanCode.append("    java.util.Set<" + codeBeanFileName + "> __tmpSet = (java.util.Set<" + codeBeanFileName
                + ">) __alreadyCopied.get(__jpaSet);\n");
        ...
        beanCode.append(
                "    " + codeBeanFileName + " __tmp = (" + codeBeanFileName + ") __alreadyCopied.get(__jpa);\n");
        ...
        for (String method : exportForeignKeys.keySet()) {
            ...
            beanCode.append("       && ( !org.fundaciobit.genapp.common.utils.Utils.isEmpty(__jpa."
                    + CodeGenUtils.getModelName(method) + ") || org.hibernate.Hibernate.isInitialized(__jpa."
                    + CodeGenUtils.get(method) + ")) ) {\n");
            beanCode.append("      __tmp." + CodeGenUtils.set(method) + "(" + jpa + "." + copyMethodName + "(__jpa."
                    + CodeGenUtils.get(method) + ", __alreadyCopied,\"" + codeBeanFileName + "\"));\n");
            beanCode.append("    }\n");
        }
    ...
    protected static String generateImportsExports(Project project, TableInfo table, FieldInfo[] fields,
            Set<String> imports, String jpaPackage, CodeType codeType) throws Exception {

        return generateImportsExports(project, table, fields, imports, jpaPackage, codeType,
                new HashMap<String, String>(), new HashMap<String, String>());

    }

    protected static String generateImportsExports(Project project, TableInfo table, FieldInfo[] fields,
            Set<String> imports, String jpaPackage, CodeType codeType, Map<String, String> importForeignKeys,
            Map<String, String> exportForeignKeys) throws Exception {
        ...
        for (int r = 0; r < fields.length; r++) {
            FieldInfo field = fields[r];
            /*
             * Class<?> typeClass = field.getJavaType(); String type = typeClass.getName();
             * String name = field.javaName;
             */
            ...
            for (ForeignKey fk : keys) {
                ...
                jpaCode.append("// EXP  Field:" + fk.getField() + " | Table: " + fk.getTable() + " | Type: "
                        + fk.getType() + "  \n\n");
                ...
                if (isOneToOne(table, field, expTable, expField)) {
                    ...
                    if (!javaNameField.endsWith("ID")) {
                        throw new Exception("El camp " + field.getJavaName() + " de la taula " + table.getNameJava()
                    ...
                    beanCode.append("    public " + pre + " void " + CodeGenUtils.set(methodname) + "(" + javaName + " "
                            + newfield + ")");
                    // TODO XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                    /*
                     * if (codeType == CodeType.INTERFACE) { interfaceCode.append(";\n"); } else
                     */ {
                        beanCode.append(" {\n");
                    ...
                    if (taulesExportadesRepetides.contains(fk.getTable())) {
                        // SI ESTA REPETIT: El camp ho feim amb el nom de la taula més el nom del camp
                        // remot
                        ...
                        if ((fileTableInfo != null && table.getName().equals(fileTableInfo.getName()))
                                || (translationTableInfo != null
                                        && table.getName().equals(translationTableInfo.getName()))) {
                    ...
                    jpaCode.append(
                            "    @OneToMany(fetch = FetchType." + fetchType + ", mappedBy = \"" + mappedBy + "\")\n");
                    ...
                    beanCode.append("    private " + collectionType1 + "<" + javaName + "> " + newfield + " = new "
                            + collectionType2 + "<" + javaName + ">(0);\n");
                    interfaceCode.append("    public " + pre + " " + collectionType1 + "<" + javaName + "> "
                            + CodeGenUtils.get(methodname));
                    if (codeType == CodeType.INTERFACE) {
                        interfaceCode.append(";\n");
                    } else {
                        beanCode.append(" {\n");
                    ...
                    interfaceCode.append("\n");
                    beanCode.append("    public void " + CodeGenUtils.set(methodname) + "(" + collectionType1 + "<"
                            + javaName + "> " + newfield + ")");
                    // TODO XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                    {
                        beanCode.append(" {\n");
            ...
            if (table.isTranslationEntity() && codeType == CodeType.JPA) {
                ...
                beanCode.append("  @ElementCollection(fetch= FetchType.EAGER, targetClass = " + jpaPackage
                        + ".TraduccioMapJPA.class)\n");
                ...
                beanCode.append("  @JoinTable(name=\"" + project.getPrefix().toLowerCase()
                        + "_traducciomap\",joinColumns={@JoinColumn(name=\"traducciomapid\")}, foreignKey=@ForeignKey(name=\""
                        + project.getPrefix().toLowerCase() + "_traducmap_traduccio_fk\"))\n");
                // XYZ ZZZ
                // beanCode.append("
                // @javax.persistence.MapKeyColumn(columns={@Column(name=\"idiomaid\")})\n");
                beanCode.append("  @javax.persistence.MapKeyColumn(name=\"idiomaid\")\n");
                // TODO FK -> Extreure de project.tables foreig keys
                // beanCode.append(" @ForeignKey(name=\"" + project.getPrefix().toLowerCase() +
                // "_traducmap_traduccio_fk\") \n");
                beanCode.append(
                        "  private Map<String, " + jpaPackage + ".TraduccioMapJPA> traduccions =  new HashMap<String, "
                                + jpaPackage + ".TraduccioMapJPA>();\n");
                beanCode.append("\n");
                beanCode.append("  public Map<String, " + jpaPackage + ".TraduccioMapJPA> getTraduccions() {\n");
                beanCode.append("    return this.traduccions;\n");
                beanCode.append("  }\n");
                beanCode.append("\n");
                beanCode.append("  public void setTraduccions(Map<String, " + jpaPackage
                        + ".TraduccioMapJPA> _traduccions_) {\n");
                ...
                beanCode.append("  \n");
                beanCode.append(
                        "  public void addTraduccio(String idioma, " + jpaPackage + ".TraduccioMapJPA traduccio) {\n");
            ...
            for (ForeignKey fk : keys) {
                ...
                jpaCode.append("// IMP Field:" + fk.getField() + " | Table: " + fk.getTable() + " | Type: "
                        + fk.getType() + "  \n\n");
                ...
                String methodname;
                /*
                 * methodname = impTable.getNameJava(); // field.getJavaName(); if
                 * (!methodname.endsWith("ID")) { throw new Exception("El camp " +
                 * field.getJavaName() + " de la taula " + table.getNameJava() +
                 * " fa referencia a una taula externa però el seu nom java no acaba amb ID"); }
                 * 
                 * // Llevam ID methodname= methodname.substring(0, methodname.length() - 2);
                 */
                ...
                if (codeType == CodeType.JPA) {
                    ...
                    if (fkName == null) {
                        fkName = project.getPrefix() + "_" + table.getNameJava().toLowerCase() + "_"
                                + expTable.getNameJava().toLowerCase() + "_fk";
                    }
                ...
                if (isTranslationTable && codeType == CodeType.JPA) {
                    ...
                    beanCode.append("  public java.util.Map<String, " + jpaPackage + ".TraduccioMapJPA> "
                            + CodeGenUtils.getOnlyName(methodname) + "Traduccions() {\n");
                    beanCode.append("    return this." + newfield + ".getTraduccions();\n");
                    beanCode.append("  }\n");
                    beanCode.append("\n");
                    beanCode.append(
                            "  public void " + CodeGenUtils.set(methodname) + "Traduccions(java.util.Map<String, "
                                    + jpaPackage + ".TraduccioMapJPA> __traduccions__) {\n");
    ...
    public static SourceFile generateCodeForIFaceManager(TableInfo tableInfo, BasicPackages packages, String jpaPackage,
            Project project, BeanCode beanCode) {
...
//    manager.append("import " + I18NException.class.getName()+ ";\n");
    ...
    public static SourceFile generateCodeForManager(TableInfo tableInfo, BasicPackages packages, String jpaPackage,
            Project project, BeanCode beanCode) {
        ...
        // XYZ ZZZ
        // manager.append("import " + GenAppPackages.PKG_BASE +
        ...
        manager.append("public class " + managerFileName + "\n         extends AbstractJPAManager<" + tableNameJava
                + ", " + pkClass + ">" + "\n         implements " + tableNameJava + "IJPAManager, I" + tableNameJava
                + "Manager, " + tableNameJava + "Fields {\n\n\n");

        manager.append("\n");

        manager.append("    public static final TableName<" + tableNameJava + "> _TABLENAME =  new TableName<"
                + tableNameJava + ">(\"" + tableNameJava + "JPA\");\n");
        manager.append("\n");

        // (2) Listener
        /*
         * XYZ ZZZ String modification = "ModificationManager<" +
         * ModelGenerator.getInterfaceName(tableNameJava)+ ">"; manager.append("\n\n");
         * // TODO llevar static !!!!!! manager.append("  static final " + modification
         * + " __eventManager =" + " new " + modification + "();\n\n");
         */
        // (0) Instance
        ...
        /*
         * manager.append("    public static " + managerFileName +
...
// XYZ ZZZ
        ...
        // (8) GetEventManager
        /*
         * XYZ ZZZ manager.append("\n"); manager.append("\n");
         * 
         * manager.append("    public ModificationManager<" + tableNameJava +
        ...
        manager.append("    public " + tableNameJava + "[] listToArray" + "(List<" + tableNameJava + "> list)  {\n");
        ...
        String[] versionParams = { dao.createParams.toString(), createEq ? null : dao.createParamsNotNull.toString() };
        ...
        for (int i = 0; i < versionBeanParams.length; i++) {
            if (versionParams[i] == null)
                continue;
            manager.append(
                    "    public " + tableNameJava + " create(" + versionParams[i] + ") throws I18NException {\n");

            manager.append(
                    "        " + className + " __bean =  new " + className + "(" + versionBeanParams[i] + ");\n");
        ...
        if (dao.pkClass.endsWith("PK")) {
            manager.append("        return __em.find(" + tableNameJava + "JPA.class, new " + dao.pkClass + "("
                    + dao.findParamsCall + "));\n");
        } else {
         ...
         * manager.append("      __query.append(\"SELECT " + instance + " FROM " +
         * className + " " + instance + " \");\n");
         * 
         ...
         * 
         * manager.
         * append("      if (firstResult != null) { q.setFirstResult(firstResult); }\n"
         ...
         * append("         log.error(\"Error executant la sentència SQL: \" + __query.toString());\n"
        ...
        if (transFields != null && transFields.size() != 0) {
            ...
            manager.append("  @Override\n");
            manager.append("  public " + tableNameJava + " create(" + tableNameJava
                    + " transientInstance) throws I18NException {\n");
            ...
            manager.append("  @Override\n");
            manager.append("  public " + tableNameJava + " update(" + tableNameJava
                    + " transientInstance) throws I18NException {\n");
            manager.append("    processTranslations(transientInstance);\n");
            ...
            for (FieldInfo fieldInfo : transFields) {
                ...
                manager.append("            } \n");
                manager.append(
                        "            transientInstance." + CodeGenUtils.set(fieldInfo) + "(_trad.getTraduccioID());\n");
                manager.append("          }\n");
                manager.append("        }\n");
                manager.append("      }\n");
            }
     ...
     * @return
     */
    public static SourceFile generateJPADaoManager(String jpaPackage, BasicPackages packages, Project project) {
    ...
    public static SourceFile generateQueryPath(BasicPackages packages, TableInfo tableInfo, FieldInfo[] fields,
            BeanCode beanCode) {
        ...
        for (int r = 0; r < fields.length; r++) {
            ...
            String fieldType = typeClass.isPrimitive() ? SQL2Java.primitiveType2javaClassName(typeClass).getName()
                    : typeClass.getName();
            ...
            // indexes.append(" getQueryPath() + " + fieldsClass + "." + name.toUpperCase()
            // + ".javaName, null);\n");
            indexes.append("  }\n");
            indexes.append("\n");
        ...
        interfaceCode.append("\n");
        interfaceCode.append("public class " + codeFileName + " extends "
                + org.fundaciobit.genapp.common.query.QueryPath.class.getName() + " {");
        interfaceCode.append("\n\n");
        ...
        interfaceCode
                .append("    return ((this.parentQueryPath == null) ? (" + fieldsClass + "._TABLE_MODEL + \".\")\n");
        ...
        for (ForeignFieldInfo item : list) {
            ...
            interfaceCode.append("      public String getQueryPath() {\n");
            interfaceCode
                    .append("          return " + codeFileName + ".this.getQueryPath() + \"" + ref + "\" + \".\";\n");
            interfaceCode.append("      }\n");
            interfaceCode.append("    });\n");
            interfaceCode.append("  }\n");
            if (item.type.equals(javax.persistence.OneToMany.class)) {
