...
import org.fundaciobit.genapp.common.validation.AbstractValidator;
...
public class JPAValidatorGenerator {
  ...
  public static List<SourceFile> generateValidator(String packageValidatorLogic,
    ...
    for (int i = 0; i < tables.length; i++) {
      ...
      final String interficie2 = "I extends " + tableJavaName;
      ...
      for (FieldInfo field : fields) {
        ...
        if (field.getJavaType().equals(String.class)) {
          ...
          // XYZ ZZZ String converter = getConverter(typeClass);***
          code.append("    if (__vr.getFieldErrorCount(" + camp + ") == 0) {\n");
          
          // TODO 
          //code.append("      " + cls + " __" + campLower + " = (" + cls  + ")__vr.getFieldValue(__target__," + camp + ");\n");
          
          // TODO XYZ ZZZ
          code.append("      " + cls + " __" + campLower + " = __target__." + CodeGenUtils.get(field) + ";\n");
          ...
          if (pattern != null) {
            if (CodeGenUtils.isInteger(pattern)) {
               // Check min len
              code.append("    if (__vr.getFieldErrorCount(" + camp + ") == 0) {\n");
              //XYZ ZZZ code.append("      " + cls + " __" + campLower + " = (" + cls + ")__vr.getFieldValue(__target__," + camp + ");\n");
              code.append("      " + cls + " __" + campLower + " = __target__."  +  CodeGenUtils.get(field) + ";\n");
      ...
      for (FieldInfo queryField : fields) {
        ...
        if (wfi.webtype == WebType.Query) {
          ...
          code.append("    if (__vr.getFieldErrorCount(" + camp + ") == 0) {\n");
          //XYZ ZZZ code.append("      " + cls + " " + campLower + " = (" + cls + ")__vr.getFieldValue(__target__," + camp + ");\n");
          code.append("      " + cls + " " + campLower + " = __target__." + CodeGenUtils.get(queryField) + ";\n");
   ...
   // =================  BASE CLASS .JAVA ==================
      ...
      */
      ...
      code.append(" */\n");
      code.append("public class " + validator + "<" +  interficie2 + ">\n");
      // XYZ ZZZ Es necessari??
      code.append("    extends " + AbstractValidator.class.getName());
      ...
      code.append("  public void validate(IValidatorResult<I> __vr," +
    		  "I __target__, boolean __isNou__");
      // EJBs
      for (String tableName : ejbs) {
  ...
  private static String genUniqueBlock(final String instanceEjb,
    ...
    if (dataGenUnique.size() != 0) {
      ...
      for (DataGenUnique dgu : all) {
        ...
        // TODO XYZ ZZZ
        // vars.append("        " + dgu.theClass + " " + dgu.campLower + " = (" + dgu.theClass  + ")__vr.getFieldValue(__target__," + dgu.camp + ");\n");
        vars.append("        " + dgu.theClass + " " + dgu.campLower + " = __target__." + CodeGenUtils.get(dgu.field) + ";\n");
  ...
  public static String converter(Class<?> typeClass) {
	  
	  Class<?> classe = typeClass.isPrimitive() ? SQL2Java.primitiveType2javaClassName(
		        typeClass) : typeClass;
	  
	  
	  return "to"+ classe.getSimpleName();
