...
public class BackWebGenerator extends IconUtils {
    ...
    public static List<SourceFile> generateListJsp(String packageControllerBack, String packageFormBack,
        ...
        for (int i = 0; i < tables.length; i++) {
...

            {
                ...
                codeMultipleSelect
                        .append("       <form:checkbox path=\"selectedItems\" aria-label=\"Seleccionar\" value=\""
                                + pkMapping.substring(1) + "\"/>\n");
...

            {
                ...
                codeHeaderMultipleSelect
                        .append("      <c:if test=\"${" + instanceFilterForm + ".visibleMultipleSelection}\">\n");
                codeHeaderMultipleSelect.append("      <th aria-label=\"Seleccionar tots\">\n");
                codeHeaderMultipleSelect.append(
                        "         <input type=\"checkbox\" aria-label=\"SeleccionarTots\" onClick=\"selectUnselectCheckBoxes(this)\" />\n");
...

            {
                ...
                codeSubtitle.append("<h6 style=\"margin-top:-10px; margin-bottom:10px; font-style:italic;\">\n");
...

            {
                ...
                codeHeaderButtons.append("<a ");
                codeHeaderButtons.append(
                        "class=\"btn btn-sm <c:out value=\"${button.style}\" /> float-right botoselecciolist\" style=\"\" "
...

            {
                ...
                final String searchByAdditional = "      <c:forEach var=\"__entry\" items=\"${__theFilterForm.additionalFields}\">\n"
                        ...
                        + "\" style=\"padding-right: 24px;padding-bottom: 4px;\">\n"
                    ...
                    try {
                        ...
                        if (Number.class.isAssignableFrom(cls) || cls.isPrimitive()) {
                            ...
                            if (field.getWebFieldInfo().getWebtype() == WebType.ComboBox) {
                                String ariaLabel = model + "." + modelCamp + "Select";
                                codeFilterBy.append("              <%-- FILTRE NUMERO SELECT MULTIPLE --%>\n"
                                        + "              <div class=\"input-group-prepend\" style=\"padding-top: 5px;padding-right: 24px;\">\n"
                                        + "              <label for=\"" + ariaLabel + "\" style=\"display: inline;\">\n"
                                        + "                 <span class=\"add-on\"><fmt:message key=\"" + model + "."
                                        + modelCamp + "\" />:</span>\n" 
                                        + "              </label>\n"
                                        + "              </div>\n" + "\n"
                                        + "              <div class=\"input-group-prepend\" style=\"min-width:200px\">\n"
                                        + "                <form:select aria-label=\"" + ariaLabel + "\"   id=\"" + id + "\" path=\"" + modelCamp
                                        + "Select\" cssClass=\"search-query input-medium form-control select2 select2-hidden-accessible\" multiple=\"true\" style=\"width:100%;\" tabindex=\"-1\" aria-hidden=\"true\">\n"
                                        + "                    <c:forEach var=\"_entry\" items=\"${__theFilterForm.mapOfValuesFor"
                                        + name + "}\">\n"
                                        + "                      <option value=\"${_entry.key}\" ${fn:contains(__theFilterForm."
                                        + modelCamp + "Select, _entry.key)?'selected':''} >${_entry.value}</option>\n"
                                        + "                    </c:forEach>\n" + "                </form:select>\n"
                                        + "              </div>\n" + "\n"
                                ...
                                String ariaLabel = model + "." + modelCamp;
                                codeFilterBy.append("            <%-- FILTRE NUMERO DESDE-FINS --%>\n");

                                codeFilterBy.append("              <label for=\"" + ariaLabel + "\" style=\"display: inline;\">\n");
                                codeFilterBy.append("              <span class=\"add-on\"><fmt:message key=\"" + model
                                        + "." + modelCamp + "\" />:</span>\n");
                                codeFilterBy.append("              </label>\n");
                                codeFilterBy.append("\n");
                                codeFilterBy.append(
                                        "              <span class=\"add-on\">&nbsp;<fmt:message key=\"genapp.from\" /></span>\n");
                                codeFilterBy.append("              \n");
                                codeFilterBy.append(
                                        "              <form:input cssClass=\"input-append input-small\" path=\""
                                                + modelCamp + "Desde\" />\n");
                                codeFilterBy.append("\n");
                                codeFilterBy.append("\n");
                                codeFilterBy.append(
                        ...
                        if (cls.equals(String.class)) {
                            ...
                            codeFilterBy.append(
                                    "              <label for=\"" + model + "." + modelCamp + "\" style=\"display: inline;\">\n");
                            ...
                            codeFilterBy.append(
                                    "              <form:input cssClass=\"search-query input-medium\" placeholder=\"${cercaper"
                                            + modelCamp + "}\" path=\"" + modelCamp + "\" aria-label=\"" + model + "." + modelCamp  + "\" />\n");
                            codeFilterBy.append("            </div>\n");
                            codeFilterBy.append("\n\n");
                            continue;
                        }
                        if (cls.equals(Time.class)) {
                            ...
                            codeFilterBy.append("            <div class=\"" + inputgroupcss
                                    + "\" style=\"padding-right:24px;padding-bottom:4px;align-items:center;\">\n");
                            codeFilterBy.append(
                                    "              <label for=\"" + ariaLabel + "\" style=\"display: inline;\">\n");
                            codeFilterBy.append("              <span class=\"add-on\"><fmt:message key=\"" + ariaLabel + "\" />:</span>\n");
                            codeFilterBy.append("              </label>\n");
                            codeFilterBy.append("\n");
                            codeFilterBy.append(
                                    "              <span class=\"add-on\">&nbsp;<fmt:message key=\"genapp.from\" /></span>\n");

                            codeFilterBy.append(generateTimePicker(modelCamp + "Desde", modelCamp + "Desde", ariaLabel));
                            ...
                            codeFilterBy.append(generateTimePicker(modelCamp + "Fins", modelCamp + "Fins", ariaLabel));
                        ...
                        if (cls.equals(Date.class)) {
                            ...
                            codeFilterBy.append(generateDatePicker(modelCamp + "Desde", modelCamp + "Desde",ariaLabel));

                            codeFilterBy.append("\n");
                            codeFilterBy.append(
                                    "              <span class=\"add-on\">&nbsp;<fmt:message key=\"genapp.to\" />&nbsp;</span>\n");

                            codeFilterBy.append(generateDatePicker(modelCamp + "Fins", modelCamp + "Fins", ariaLabel));
                        ...
                        if (cls.equals(Timestamp.class)) {
                            final String ariaLabel = model + "." + modelCamp;
                            ...
                            codeFilterBy.append(generateDateTimePicker(modelCamp + "Desde", modelCamp + "Desde", ariaLabel));

                            codeFilterBy.append(
                                    "              <span class=\"add-on\">&nbsp;<fmt:message key=\"genapp.to\" />&nbsp;</span>\n");

                            codeFilterBy.append(generateDateTimePicker(modelCamp + "Fins", modelCamp + "Fins", ariaLabel));
            ...
            String testDeFerVisibleBotons = "${" + instanceFilterForm + ".editButtonVisible " + "|| "
                    + instanceFilterForm + ".viewButtonVisible " + "||" + instanceFilterForm + ".deleteButtonVisible"
                    + " || not empty " + instanceFilterForm + ".additionalButtonsForEachItem" + " || not empty "
                    + instanceFilterForm + ".additionalButtonsByPK}";
            {
            ...
            // ============================ LIST_BUTTONS
            {
                ...
                for (ActionsRenderer render : renderers) {
                    ...
                    final String commonCodeAddBut = "                  <c:set var=\"thehref\" value=\"#\"/>\n"
                            ...
                            + render.getActionButtonCode("                  ", "<c:out value=\"${button.style}\" />",
                                    "${thehref}", "${thelink}", "${button.icon}", "${button.codeText}");
            ...
            for (FieldInfo field : fields) {
                ...
                try {
                    ...
                    switch (field.getWebFieldInfo().getWebtype()) {
                         ...
                         */
                        {
                            ...
                            if (fk == null && field.getWebFieldInfo().getWebtype() != WebType.ComboBox) {
                                ...
                                if (transTable != null && fk != null && transTable.getName().equals(fk.getTable())) {
                                    ...
                                    codeCoreContent.append("          <c:set var=\"tmp\">${" + model + "." + modelCamp
                                            + "}</c:set>\n");
                                    ...
                                    codeCoreContent.append("${" + instanceFilterForm + ".mapOf" + fkTableName + "For"
                                            + name + "[tmp]}");
                                    /*
                            ...
                            if (field.getMinAllowedValue() != null) {
                                ...
                                codeCoreContent.append(
                                        "            <img height=\"18\" width=\"18\" src=\"<c:url value=\"/img/icn_alert_${"
                                                + model + "." + modelCamp + "?'success':'error'}.png\"/>\" alt=\"${"
                                                        + model + "." + modelCamp + "}\" >\n");
                                if (!field.isNotNullable()) {
                                    codeCoreContent.append("            </c:if>\n");
                                }
                        ...
                        case WebType.Date:
                            codeCoreContent.append(
                                    "          <td> <fmt:formatDate pattern=\"${gen:getDatePattern()}\" value=\"${"
                                            + model + "." + modelCamp + "}\" /></td>\n");
                            continue;

                        case WebType.Time:
                            codeCoreContent.append(
                                    "          <td> <fmt:formatDate pattern=\"${gen:getTimePattern()}\" value=\"${"
                                            + model + "." + modelCamp + "}\" /></td>\n");
                            continue;
                        ...
                        default:
                            throw new Exception("NO existix generador de LIST_JSP pel camp " + field.javaName
                                    + " de tipus " + field.getClass());
    ...
    public static abstract class ActionsRenderer {
        ...
        public String getImage(String icon, String tab, String codeMessage) {
            if (icon.trim().startsWith("${")) {
                ...
                code.append(tab + "<img src=\"<c:url value=\"" + icon + "\"/>\" alt=\"" + codeMessage + "\"/>\n");
    ...
    public static class ActionsRendererDropDownButton extends ActionsRenderer {
        ...
        public String getHeader(String instanceFilterForm) {
            ...
            return "    <div class=\"btn-group\">\n" + "      <a class=\"btn btn-sm " + " ${(empty " + botomenu
                    ...
                    + "} dropdown-toggle\" data-toggle=\"dropdown\" href=\"#\" aria-label=\"Opcions\">"
    ...
    public static class ActionsRendererSimpleIconList extends ActionsRenderer {
        ...
        public String getActionButtonCode(String tab, String type, String href, String onclick, String icon,
            ...
            codeButtons.append(getImage(icon, tab + "   ",  codeMessage) + "\n");
    ...
    */
...

            {
                ...
                codeButton.append(
                        "    <input type=\"button\" class=\"btn btn-secondary\" onclick=\"goTo('<c:url value=\"${contexte}"
                                + ""
                                + pkMapping.substring(0, pkMapping.length() - 1).replace("/${",
                                        "${" + instanceForm + ".nou?'':'/'.concat(")
                                + ")}/cancel\"/>')\" value=\"<fmt:message key=\"genapp.cancel\"/>\">\n");
                codeButton.append("    </c:if>\n");
                // BOTO DELETE
                ...
                // style=\"${(empty button.style)? '' : 'color: white;'}\" 
            ...
            // ============= JSP FORM CORE
            {
                ...
                for (FieldInfo field : fields) {
                    ...
                    if (codeField != null) {
                        ...
                        code.append("            <label for=\"" + model + "." + modelCamp + "\">\n");
    ...
    private static String generateFieldFormInput(final String model, final String instanceForm,
        ...
        if (fileFields.contains(field)) {
            ...
            code.append("                  <label class=\"custom-file-label\" for=\"" + modelCamp + "\" aria-label=\"Browser\">\n");
            ...
            code.append("                  </span>\n");
            if (!field.isNotNullable) {
                code.append("                  <span class=\"input-group-text\" >\n");
                code.append("                        <form:checkbox aria-label=\"" + model + "." + modelCamp + "Delete\"  path=\"" + modelCamp + "Delete\"/>\n");
                code.append("                        <label style=\"margin-bottom:0px\" for=\"" + model + "." + modelCamp + "Delete\">\n");
                code.append("                        <small style=\"color:#000\">&nbsp;<fmt:message key=\"genapp.form.file.delete\"/></small>\n");
                code.append("                        </label>\n");
                code.append("                  </span>\n");
        ...
        if (translationFields.contains(field)) {
            ...
            type = type + " aria-label=\"" + model + "." + modelCamp +  "\" ";
        ...
        switch (webType) {
            ...
            case WebType.Decimal:
                ...
                if (webType == WebType.URL) {
                    ...
                    code.append("               <a href=\"${" + instanceForm + "." + model + "." + modelCamp
                            + "}\" target=\"_blank\">" + "${" + instanceForm + "." + model + "." + modelCamp
                            + "}</a>\n\n");
                    code.append("             </c:if>\n");
                    code.append("           </c:if>\n\n");
            ...
            case WebType.Query:
                ...
                code.append(
                        "          <input type=\"text\" readonly=\"true\" class=\"form-control col-md-9-optional uneditable-input\" "
                                + "value=\"${gen:findValue(" + instanceForm + "." + model + "." + modelCamp + "," + instanceForm
                        + ".listOf" + fkTableName + "For" + name + ")}\" aria-label=\"" + model + "." + modelCamp +  "\" />\n");
                code.append("          </c:if>\n");
                code.append("          <c:if test=\"${!" + readOnlyCondition + "}\" >\n");
                ...
                if (!field.isNotNullable()) {
                    ...
                    code.append("              </c:if>\n");
                    code.append(
                            "              <c:if test=\"${not empty __theForm." + model + "." + modelCamp + " }\">\n");
            ...
            case WebType.RichText:
                ...
                if (webType == WebType.TextArea) {
                    ...
                    code.append("        <button  class=\"btn btn-secondary btn-sm dropdown-toggle\" type=\"button\""
                            + " style=\"margin-left:0px;\" aria-label=\"Opcions de format\">"
                            + "<span class=\"caret\"></span></button>\n");
                    code.append("        <div id=\"dropdownMenuContainer_" + modelCamp
                            + "\" class=\"dropdown-menu dropdown-menu-right\">\n");
                    code.append("          <a class=\"dropdown-item\" href=\"#\" "
                            + "onclick=\"javascript:var ta=document.getElementById('" + path
                            + "'); ta.wrap='off';\" >No Wrap</a>\n");
                    code.append("          <a class=\"dropdown-item\"  href=\"#\" "
                            + "onclick=\"javascript:var ta=document.getElementById('" + path
                            + "'); ta.wrap='soft';\">Soft Wrap</a>\n");
                    code.append("          <a class=\"dropdown-item\" href=\"#\" "
                            + "onclick=\"javascript:var ta=document.getElementById('" + path
                            + "'); ta.wrap='hard';\">Hard Wrap</a>\n");
                    code.append("        </div>\n");
                    code.append("      </div>\n");
                    code.append("      <script type=\"text/javascript\">\n");
            ...
            case WebType.DateTime: {
                ...
                final String ariaLabel = pathID;
                code.append("    <form:errors path=\"" + pathID + "\" cssClass=\"errorField alert alert-danger\" />\n");
                String componentID = model + "_" + modelCamp;
                code.append(generateCommonDateTimePicker(webType, readOnlyCondition, readOnlyAttribute, componentID,
                        pathID, ariaLabel));
                return code.toString();
            }
            ...
            default:
                throw new Exception("NO existix generador de FORM_JSP pel camp " + field.javaName + " de tipus "
                        + field.getClass());
    ...
    protected static String generateTimePicker(String componentID, String path, String ariaLabel) throws Exception {
        final int webType = WebType.Time;
        final String readOnlyCondition = "false";
        final String readOnlyAttribute = "";
        return generateCommonDateTimePicker(webType, readOnlyCondition, readOnlyAttribute, componentID, path, ariaLabel);
    }

    protected static String generateDateTimePicker(String componentID, String path, String ariaLabel) throws Exception {
    ...
    protected static String generateDatePicker(String componentID, String path, String ariaLabel) throws Exception {
    ...
    protected static String generateCommonDateTimePicker(int webType, final String readOnlyCondition,
            final String readOnlyAttribute, String componentID, String path, String ariaLabel) throws Exception {
        ...
        code.append("                      <form:input " + readOnlyAttribute
                ...
                + " path=\"" + path + "\" aria-label=\"" + ariaLabel + "\"  />\n");
        code.append("                    <c:if test=\"${!" + readOnlyCondition + "}\" >\n");
