...
public class BackGenerator {
    ...
    public static List<SourceFile> generateValidator(String packageValidatorBack, String packageFormBack,
            String jpaPackage, String ejbPackage, String logicValidationPkg, BasicPackages packages, Project project)
            throws Exception {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            final String interfaceBeanModel = tableJavaName;
            final String baseFormulari = CodeGenUtils.getModelName(tableJavaName);
            ...
            code.append("import org.fundaciobit.genapp.common.validation.BeanValidatorResult;\n");
            code.append("import org.fundaciobit.genapp.common.i18n.I18NFieldError;\n");
            ...
            code.append("import " + AbstractWebValidator.class.getName() + ";\n");
            code.append("import " + packages.entityPackage + "." + interfaceBeanModel + ";\n");
            ...
            code.append("public class " + validator + " extends " + AbstractWebValidator.class.getSimpleName() + "<"
                    + form + ", " + interfaceBeanModel + ">");
            code.append("\n     implements Validator, " + fieldsClass + " {\n");
            code.append("\n");
            code.append("     protected final Logger log = Logger.getLogger(getClass());\n");
            code.append("\n");

            code.append("  protected " + logicValidator + "<" + interfaceBeanModel + "> validator = new "
                    + logicValidator + "<" + interfaceBeanModel + ">();\n");
            ...
            */
            ...
            code.append("  @Override\n");
            code.append("  public " + interfaceBeanModel + " getBeanOfForm(" + form + " form) {\n");
            code.append("    return  form." + CodeGenUtils.get(tableJavaName) + ";\n");
            code.append("  }\n");
            code.append("\n");

            code.append("  @Override\n");
            code.append("  public Class<" + form + "> getClassOfForm() {\n");
            code.append("    return " + form + ".class;\n");
            code.append("  }\n");
            code.append("\n");

            code.append("  @Override\n");
            code.append("  public void validate(" + form + " __form, " + interfaceBeanModel
                    + " __bean, Errors errors) {\n");
            code.append("\n");
            ...
            code.append("    WebValidationResult<" + form + "> wvr;\n");
            code.append("    wvr = new WebValidationResult<" + form + ">(errors");
            ...
            code.append("    boolean isNou;\n");
            code.append("    {\n");
            code.append("        Object objNou = errors.getFieldValue(\"nou\");\n");
            ...
            code.append("\n");
            code.append("    validate(__form, __bean , errors, wvr, isNou);\n");

            code.append("  }\n");
            code.append("\n\n");

            code.append(
                    "  public void validate(" + form + " __form, " + interfaceBeanModel + " __bean, Errors errors,\n");
            code.append("    WebValidationResult<" + form + "> wvr, boolean isNou) {\n\n");

            if (transFields != null && transFields.size() != 0) {
                code.append("  {\n");

                code.append("      " + jpaPackage + "." + table.getNameJava() + "JPA __jpa;\n");
                code.append("      __jpa = (" + jpaPackage + "." + table.getNameJava() + "JPA)__bean;\n");

                /*
                code.append("    if (__form instanceof " + form + ") {\n");
                code.append("      " + model + " = ((" + form + ")__form)." + CodeGenUtils.get(model)+ ";\n");
                code.append("    } else {\n");
                code.append("      " + model + " = (" + jpaPackage+ "."+ table.getNameJava()+"JPA)__form;\n");
                code.append("    }\n");
                */
                ...
                for (FieldInfo fieldInfo : transFields) {
                    ...
                    code.append("      " + jpaPackage + ".TraduccioJPA tradJPA = __jpa."
                            + CodeGenUtils.get(fieldInfo).replace("ID()", "()") + ";\n");
                    ...
                    String path = baseFormulari + "."
                            + fieldInfo.getJavaName().substring(0, fieldInfo.getJavaName().length() - 2);
                    String codeError = "\"genapp.validation.required\", new String[] {" + I18NUtils.class.getName()
                            + ".tradueix(" + fieldInfo.getJavaName().toUpperCase() + ".fullName)}, null);\n";

                    code.append("        java.util.Map<String," + jpaPackage
                            + ".TraduccioMapJPA> _trad = tradJPA.getTraduccions();\n");
                    if (isNullable) {
                    ...
                    if (isNullable) {
                        ...
                        code.append("          __jpa." + setMethod + "(null);\n");
                        code.append("          __jpa." + setMethod.substring(0, setMethod.length() - 2) + "(null);\n");
                        code.append("        } else {\n");
                    }
                    ...
                    code.append("                errors.rejectValue(\"" + path
                            + ".traduccions[\"+ _idioma +\"].valor\",\n");
            ...
            code.append("    BeanValidatorResult<" + interfaceBeanModel + "> __vr = new BeanValidatorResult<"
                    + interfaceBeanModel + ">();\n");
            ...
            code.append("\n");
            code.append("    if (__vr.hasErrors()) {\n");
            code.append("        List<I18NFieldError> vrErrors = __vr.getErrors();\n");
            code.append("    	   for (I18NFieldError i18nFieldError : vrErrors) {\n");
            code.append(
                ...
                for (FieldInfo field : fileFields) {
                    ...
                    if (field.isNotNullable() && !field.isAutoIncrement()) {
                        ...
                        //if (fileFields.contains(field)) 
                        {
                            ...
                            code.append("            CommonsMultipartFile " + field.javaName + " = ((" + form
                                    + ")__form)." + CodeGenUtils.get(field) + ";\n");
                            code.append("            if (" + field.javaName + " == null || " + field.javaName
                                    + ".isEmpty()) {\n");
                            code.append("                errors.rejectValue(get(" + camp
                                    + "), \"genapp.validation.required\",\n");
                            code.append("                new String[]{ " + I18NUtils.class.getName() + ".tradueix(get("
                                    + camp + ")) },\n");
                            code.append("                null);\n");
                            code.append("            }\n");
                            code.append("        }\n");
            ...
            code.append("\n");
            code.append("  public " + logicValidator + "<" + interfaceBeanModel + "> getValidator() {\n");
            code.append("    return validator;\n");
            code.append("  }\n");
            code.append("\n");
            code.append("  public void setValidator(" + logicValidator + "<" + interfaceBeanModel + "> validator) {\n");
    ...
    protected static String checkNumberMaxMin(TableInfo table, FieldInfo field, boolean checkMinim) throws Exception {
        ...
        try {
            ...
            throw new Exception("El valor " + (checkMinim ? "minim" : "màxim") + " permes (get"
                    + (checkMinim ? "Min" : "Max") + "AllowedValue) del camp " + field.javaName + " de la classe "
                    + table.getNameJava() + " no es convertible a numero (" + valueStr + ")");
        }
        String camp = field.javaName.toUpperCase();
        ...
        String cls = typeClass.isPrimitive() ? SQL2Java.primitiveType2javaClassName(typeClass).getName()
                : typeClass.getName();
        StringBuffer code = new StringBuffer();
        code.append("    if (errors.getFieldErrorCount(get(" + camp + ")) == 0) {\n");
        code.append("      " + cls + " __" + campLower + " = (" + cls + ")errors.getFieldValue(get(" + camp + "));\n");

        String comparation = " new java.math.BigDecimal(__" + campLower
                + ".toString()).compareTo(new java.math.BigDecimal(\"" + value + "\"))";
        String errorMsgCode;
        if (checkMinim) {
        ...
        code.append("        errors.rejectValue(get(" + camp + "), \"" + errorMsgCode + "\",\n");
        code.append("            new String[]{ String.valueOf(__" + campLower + "), " + "I18NUtils.tradueix(get(" + camp
    ...
    public static List<SourceFile> generateForms(String packageForm, String packageFormBack, BasicPackages packages,
            String packageJPA, Project project) {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            List<FieldInfo> transFields = CodeGenUtils.getTranslationFieldsOfTable(tables, table);
            List<String> refsCamps = generateReferencesForForm(project, table, fileTable, transFields, refs, prefix,
                    refType);
            ...
            */
            ...
            if (transFields != null && transFields.size() != 0) {
                ...
                str.append("  }\n");
                str.append("\n");
                str.append("  public void setIdiomesTraduccio(java.util.List<" + packages.entityPackage
                        + ".Idioma> idiomesTraduccio) {\n");
    ...
    public static List<String> generateReferencesForForm(Project project, TableInfo table, TableInfo fileTable,
            List<FieldInfo> transFields, StringBuffer refs, final String prefix, final String refType) {
        ...
        for (FieldInfo field : table.getFields()) {
            ...
            if (wfi.getWebtype() == WebType.Query || wfi.getWebtype() == WebType.ComboBox) {
                ...
                // Si és fitxer omitim
                if (fileTable != null && wfi.getForeignKey() != null
                        && fileTable.getName().equals(wfi.getForeignKey().getTable())) {
                    continue;
                }
    ...
    protected static String generateReferenceMethodName(final String prefix, FieldInfo field, TableInfo fkTable) {
        String name;
    ...
    public static List<SourceFile> generateFilterForms(String packageForm, String packageFormBack, String jpaPackage,
            String ejbPackage, BasicPackages packages, TableInfo[] tables, Project project) throws Exception {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            code.append(" */\n");
            code.append("@Component\n");
            code.append("public class " + form + " extends " + project.getProjectName() + "BaseFilterForm implements "
                    + fieldsClass + " {\n");
            code.append("\n");
            ...
            for (FieldInfo field : table.getFields()) {
                ...
                if (type.equals(String.class)) {
                    ...
                    code.append(
                            CodeGenUtils.generateGetSetBean(new FieldInfo(field.getJavaName(), field.getJavaType())));
                } else {
                    ...
                    if (field.getWebFieldInfo().getWebtype() == WebType.ComboBox) {
                        ...
                        code.append(
                                CodeGenUtils.generateGetSetBean(name, "java.util.List<" + type.getName() + ">", true));
            ...
            code.append("    return new java.util.ArrayList<Field<?>>(Arrays.asList(new Field<?>[] { "
                    + nomsFilter.toString().toUpperCase() + " }));\n");
            code.append("  }\n");
            code.append("\n");
            code.append("  @Override\n");
    ...
    public static List<SourceFile> generateControllers(String backPackage, String packageControllerBase,
            String packageControllerBack, String packageFormBack, String packageValidatorBack, String jpaPackage,
            String ejbPackage, BasicPackages packages, Project project, Map<String, BeanCode> beanCodeBySqlTableName)
            throws Exception {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            if (multiplePKs) {
                ...
                for (FieldInfo fieldInfo : PKs) {
                    ...
                    pkParams = pkParams + "@PathVariable(\"" + map + "\") " + fieldInfo.getJavaType().getName() + " "
                            + map;
            ...
            for (FieldInfo field : fields) {
                ...
                if (wfi.getWebtype() == WebType.Checkbox && (type.equals(Boolean.class) || type.equals(Boolean.TYPE))) {
                    ...
                    codeTmp.append("      fillValuesToGroupByItemsBoolean(\"" + prefix + "\", groupByItemsMap, "
                            + field.javaName.toUpperCase()
                ...
                codeTmp.append("    {\n");
                codeTmp.append("      _listSKV = getReferenceListFor" + name
                        + "(request, mav, filterForm, list, groupByItemsMap, null);\n");
                ...
                codeExportList.append("    __mapping.put(" + field.getJavaName().toUpperCase() + ", filterForm."
                        + generateReferenceMethodName("getMapOf", field, fkTable) + "());\n");

                codeTmp.append("      filterForm." + setmethod + "(_tmp);\n");
                codeTmp.append(
                        "      if (filterForm.getGroupByFields().contains(" + field.javaName.toUpperCase() + ")) {\n");
            ...
            code.append(" * @author GenApp\n");
            code.append(" */\n");
            code.append("@MenuOption(labelCode=\"" + model + "." + model + ".plural\", order=" + (i * 10)
                    + ", group=Tab.MENU_WEBDB)\n");
            ...
            code.append("    ff = (" + filterForm
                    + ") request.getSession().getAttribute(getSessionAttributeFilterForm());\n");
            ...
            code.append("    " + instanceFilterForm + " = (" + filterForm
                    + ") request.getSession().getAttribute(getSessionAttributeFilterForm());\n");
            ...
            code.append("   */\n");
            code.append(
                    "  protected List<" + tableJavaName + "> llistat(ModelAndView mav, HttpServletRequest request,\n");
            ...
            code.append("    fillReferencesForList(filterForm,request, mav, " + model
            ...
            if (transFields != null) {
                for (FieldInfo fieldInfo : transFields) {
                    ...
                    code.append("    if (" + instanceForm + ".get" + tableJavaName + "()."
                            + getmethod.substring(0, getmethod.length() - 4) + "() == null){\n");
                    code.append(
                            "      " + jpaPackage + ".TraduccioJPA trad = new " + jpaPackage + ".TraduccioJPA();\n");
                    code.append("      for (" + packages.entityPackage + ".Idioma idioma : " + instanceForm
                            + ".getIdiomesTraduccio()) {\n");
                    code.append("        trad.addTraduccio(idioma.getIdiomaID(), new " + jpaPackage
                            + ".TraduccioMapJPA());\n");
                    code.append("      }\n");

                    String setmethod = CodeGenUtils.set(fieldInfo); // -2 == Llevar ID
                    code.append("      " + instanceForm + ".get" + tableJavaName + "()."
                            + setmethod.substring(0, setmethod.length() - 2) + "(trad);\n");
                    code.append("    }\n");
                    code.append("\n");
                }
            }
            ...
            for (FieldInfo field : fields) {
                ...
                if (translateTable != null && wfi.getForeignKey() != null
                        && translateTable.getName().equals(wfi.getForeignKey().getTable())) {
                    continue;
                }
                ...
                */
                ...
                code.append("      List<StringKeyValue> _listSKV = getReferenceListFor" + name + "(request, mav, "
                        + instanceForm + ", null);\n\n");
            ...
            String validationException = "      if (__e instanceof " + I18NValidationException.class.getSimpleName()
                    + ") {\n" + "        ValidationWebUtils.addFieldErrorsToBindingResult(result, ("
                    + I18NValidationException.class.getSimpleName() + ")__e);\n" + "        return getTileForm();\n"
                    + "      }\n";

            if (transFields != null && transFields.size() != 0) {
                code.append("\n");
                code.append("\n");
                code.append("  public List<" + packages.entityPackage
                        + ".Idioma> getIdiomesSuportats() throws I18NException {\n");
                code.append("    List<" + packages.entityPackage + ".Idioma> idiomes = idiomaEjb.select("
                        + packages.fieldsPackage + ".IdiomaFields.SUPORTAT.equal(true));\n");
            ...
            code.append("\n");
            if (fileFields.size() != 0) {
                code.append("    FilesFormManager<" + fileTable.getNameJava()
                        + "> afm = getFilesFormManager(); // FILE\n\n");
            }
            code.append("    try {\n");
            if (fileFields.size() != 0) {
            ...
            code.append("    } else {\n");
            code.append("      ModelAndView mav = new ModelAndView(getTileForm());\n");
            code.append("      " + form + " " + instanceForm + " = get" + form + "(" + model
                    + ", __isView, request, mav);\n");
            code.append("      " + instanceForm + ".setView(__isView);\n");
            code.append("      if(__isView) {\n");
            code.append("        " + instanceForm + ".setAllFieldsReadOnly(ALL_" + tableJavaName.toUpperCase()
                    + "_FIELDS);\n");
            ...
            // XYZ ZZZ   Quna es passi a model d'annotacions llavors afegir  @Valid devora  @ModelAttribute
            ...
            if (fileFields.size() != 0) {
                ...
                code.append(
                        "    FilesFormManager<" + fileTable.getNameJava() + "> afm = getFilesFormManager(); // FILE\n");
            }
            code.append("    try {\n");
            if (fileFields.size() != 0) {
                code.append("      this.setFilesFormToEntity(afm, " + model + ", " + instanceForm + "); // FILE\n");
            ...
            code.append("      if (" + model + " == null) {\n");
            code.append(
                    "        String __msg = createMessageError(request, \"error.notfound\", " + pkFromParams + ");\n");
            ...
            code.append(
                    "      String msg = createMessageError(request, \"error.deleting\", " + pkFromParams + ", e);\n");
            ...
            if (multiplePKs) {
                code.append(
                        " throw new Exception(\"Classes with multiple PK, must overwrite method deleteSelected()\");\n");
                code.append("/*");
            }
            ...
            code.append("      redirect = eliminar" + tableJavaName
                    + "(stringToPK(seleccionats[i]), request, response);\n");
            code.append("    }\n");
            code.append("  }\n");
            ...
            if (!multiplePKs) {
                ...
                if (pk.getJavaType().equals(String.class)) {
                    ...
                    code.append("  return " + pkClass + ".parse" + pkClass.substring(pkClass.lastIndexOf('.') + 1)
                            + "(value, 10);\n"); // TODO
                }
                code.append("}\n");
                code.append("\n");
            ...
            */
            ...
            code.append("  public String cancel" + tableJavaName
            ...
            if (fileFields.size() != 0) {
                code.append("  // FILE\n");
                code.append("  @Override\n");
                code.append("  public void setFilesFormToEntity(FilesFormManager<" + fileTable.getNameJava() + "> afm, "
                        + tableJavaName + " " + model + ",\n      " + form + " form");
                ...
                for (FieldInfo fieldInfo : fileFields) {
                    ...
                    code.append("    f = (" + fileTable.getNameJava() + "JPA)afm.preProcessFile(form."
                            + CodeGenUtils.getOnlyName(jn) + "(), form." + CodeGenUtils.isOnlyName(jn) + "Delete(),\n");
                    ...
                    code.append(
                            "        form.isNou()? null : " + model + "." + CodeGenUtils.getOnlyName(jn) + "());\n");
            ...
            for (FieldInfo field : fields) {
                ...
                if (transFields != null && !transFields.contains(field)) {
                    ...
                    code.append(
                            "    if (" + instanceForm + ".isHiddenField(" + field.javaName.toUpperCase() + ")) {\n");
                    ...
                    if (wfi.getWebtype() == WebType.Query) {
                        ...
                        code.append("    if (" + instanceForm + ".isReadOnlyField(" + field.javaName.toUpperCase()
                                + ")) {\n");
                        code.append("      _where = " + fkTable.getNameJava() + "Fields."
                                + fkInfo.getJavaName().toUpperCase() + ".equal(" + instanceForm + ".get" + tableJavaName
                                + "().get" + name + "());\n");
                        code.append("    }\n");
                        code.append("    return getReferenceListFor" + name
                                + "(request, mav, Where.AND(where, _where));\n");
                    } else {
                ...
                code.append("  public List<StringKeyValue> getReferenceListFor" + name + "("
                        + "HttpServletRequest request,\n" + "       ModelAndView mav, " + filterForm + " "
                        + instanceFilterForm + ",\n" + "       List<" + tableJavaName
                        + "> list, Map<Field<?>, GroupByItem> _groupByItemsMap"
                        + ", Where where)  throws I18NException {\n");
                ...
                code.append(
                        "       && !" + instanceFilterForm + ".isGroupByField(" + field.javaName.toUpperCase() + ")");
                if (wfi.getWebtype() == WebType.ComboBox) {
                    code.append("\n       && !" + instanceFilterForm + ".isFilterByField("
                            + field.javaName.toUpperCase() + ")");
                }
                code.append(") {\n");
                code.append("      return EMPTY_STRINGKEYVALUE_LIST;\n");
                ...
                if (wfi.getWebtype() == WebType.Query) {
                    ...
                    code.append("      java.util.Set<" + type.getName() + "> _pkList = new java.util.HashSet<"
                            + type.getName() + ">();\n");
                    ...
                    code.append("        _w = " + fkTable.getNameJava() + "Fields." + fkInfo.getJavaName().toUpperCase()
                            + ".in(_pkList);\n");
                ...
                if (wfi.getWebtype() == WebType.Query) {
                    code.append("    return " + CodeGenUtils.getModelName(fkTable.getNameJava())
                            + "RefList.getReferenceList(" + fkTable.getNameJava() + "Fields."
                            // + fk.getField().toUpperCase()
                            + fkInfo.getJavaName().toUpperCase() + ", where );\n");
                } else {
                    ...
                    if (field.getJavaType().equals(Boolean.class) || field.getJavaType().equals(Boolean.TYPE)) {
                        ...
                        if (values == null) {
                            ...
                            if (split.length != 1) {
                                ...
                                throw new Exception("El camp " + field.javaName + " de la taula " + tableJavaName
                                        + ", que es de tipus booleà, en l'apartat 'MinAllowedValue' "
                        ...
                        code.append("    __tmp.add(new StringKeyValue(\"false\" , I18NUtils.tradueix(\"" + baseTraduccio
                                + ".false\")));\n");
                        code.append("    __tmp.add(new StringKeyValue(\"true\" , I18NUtils.tradueix(\"" + baseTraduccio
                                + ".true\")));\n");
                        ...
                        if (values == null || values.trim().length() == 0) {
                            throw new Exception("El camp " + field.javaName + " de la taula " + tableJavaName
                                    + " ha de definir la llista de valors separats per comes "
                        ...
                        for (int j = 0; j < split.length; j++) {
                            ...
                            String key = split[j];
                            String value;
                            if (key.endsWith("]") && key.lastIndexOf('[') != -1) {
                                int pos = key.lastIndexOf('[');
                                // TODO Canviar per commons-text
                                value = key.substring(pos + 1, key.length()-1);
                                key = key.substring(0, pos);
                            } else {
                                value = key;
                            }

                            code.append("    __tmp.add(new StringKeyValue(\"" + StringEscapeUtils.escapeJava(key)
            ...
            */
            ...
            code.append("\n");
            code.append("  public void preList(HttpServletRequest request," + " ModelAndView mav, " + filterForm
                    + " filterForm)  throws I18NException {\n");
            code.append("  }\n");

            code.append("\n");
            code.append("  public void postList(HttpServletRequest request, " + "ModelAndView mav, " + filterForm
                    + " filterForm, " + " List<" + tableJavaName + "> list) throws I18NException {\n");
            code.append("  }\n");

            code.append("\n");
            code.append("  public String getRedirectWhenCreated(HttpServletRequest request, " + form + " "
                    + instanceForm + ") {\n");
            code.append("    return \"redirect:\" + getContextWeb() + \"/list/1\";\n");
            code.append("  }\n");

            code.append("\n");
            code.append("  public String getRedirectWhenModified(HttpServletRequest request, " + form + " "
                    + instanceForm + ", Throwable __e) {\n");
            ...
            code.append("\n");
            code.append("  public String getRedirectWhenDelete(HttpServletRequest request, " + pkParamsSense
                    + ", Throwable __e) {\n");
            code.append("    return \"redirect:\" + getContextWeb() + \"/list\";\n");
            code.append("  }\n");
            ...
            code.append("  public " + jpa + " findByPrimaryKey(HttpServletRequest request, " + pkParams2.trim()
            ...
            code.append("\n");
            code.append("  public void delete(HttpServletRequest request, " + tableJavaName + " " + model
                    + ") throws I18NException {\n");
