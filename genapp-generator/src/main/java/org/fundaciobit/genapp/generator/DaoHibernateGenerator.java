...
//    manager.append("import " + GenAppPackages.PKG_BASE + ".common.events.ModificationManager;\n");
        ...
        // (0) Instance
        manager.append("  private static final " + managerFileName + " __manager = new " + managerFileName + "();\n\n");
        ...
        for (int i = 0; i < versionBeanParams.length; i++) {
            if (versionParams[i] == null)
                continue;
            manager.append("\tpublic synchronized " + tableNameJava + " create(" + versionParams[i]
                    + ") throws Exception {\n");

            manager.append("\t\t" + className + " __bean =  new " + className + "(" + versionBeanParams[i] + ");\n");
        ...
        manager.append("  try {\n");
        manager.append("    transaction = session.beginTransaction();\n");
        manager.append("    " + tableNameJava + " __item = (" + tableNameJava + ") session.get(" + tableNameJava
                + ".class, " + dao.findParamsCall + ");\n");
