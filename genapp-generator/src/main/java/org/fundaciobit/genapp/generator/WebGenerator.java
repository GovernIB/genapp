...
public class WebGenerator {
    ...
    public static SourceFile generateWebXml(String projectName, String packageWeb) {
        return new SourceFile("web.xml", "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n"
                ...
                + packageWeb + "." + getStatupServletName(projectName) + "</servlet-class>\n"
    ...
    public static SourceFile generateStartupServlet(String projectName, String packageName, String webPackName) {
        String name = getStatupServletName(projectName);
        return new SourceFile(name + ".java", "package " + webPackName + ";\n\n" + "import javax.servlet.*;\n"
                ...
                + "      super.init(config);\n" + "      " + getInitializationClassName(projectName)
                + ".init(config);\n" + "    }\n\n" + " }\n");
    }
     ...
     * @return
     */
    public static SourceFile generateInitializationClass(String projectName, String packageBase,
            String modelPackageName, String webPackName) {
        ...
        return new SourceFile(name + ".java", "package " + webPackName + ";\n" + "import java.io.File;\n"
                ...
                + "  // 2.-Establir DAO \n" + "  " + projectName + "DaoManager.setDaoManagers(new " + projectName
                + "SqlDaoManagers());" + "  \n" + "  // 3.-Establir Seguretat \n" + "  ISecurity sec;\n" + "  \n"
                + "  // ============ DB SECURITY\n" + "  // Des de memoria\n" + "  sec = " + managerSec
                + ".getDefaultDBSecurity();\n" + "  // Des de fitxer\n" + "  //sec = " + managerSec
                + ".getDBSecurityFromFile(webInfDir); \n" + "  \n" + "  // ============ LDAP SECURITY\n"
                ...
                + "  } catch (Exception e) {\n" + "    e.printStackTrace();\n" + "  }\n" + "  \n" + "  " + managerSec
                + ".setSecurity(sec); // SET VALUE  - MOLT IMPORTANT !!!!!\n" + "  \n" + "  } catch(Exception e) {"
    ...
    public static SourceFile generateTableManager(Project project) {
        ...
        ConditionalColumn[] columns = { new ConditionalColumn("Nom", null), new ConditionalColumn("Afegir", null),
        ...
        for (int x = 0; x < tables.length; x++) {
            ...
            htmlCode[pos][1] = "<a href=\"" + t + "_New.jsp\"><img src=\"./images/new.gif \" /> </a>";
            htmlCode[pos][2] = "<a href=\"" + t + "_List.jsp\"><img src=\"./images/list.gif \" /> </a>";
            pos++;
        }
        html.imports.add(GenAppPackages.PKG_BASE + ".common.security.*");
        html.imports.add(GenAppPackages.PKG_BASE + ".common.query.*");
        html.tableGenerator = new MultiColumnMatrixTableGenerator(title, columns, htmlCode);

        html.rolesOfPage = new String[] { "getSecurity().getAdminRole()" };
        ;

        return new SourceFile(HtmlPageGenerator.TableManagerJsp, html.generateHtmlPage());

    }

    private static boolean generatedRichTextJavascript = true;

    private static final String EXECUTEQUERYMETHODNAME(String projectName, String valueIfNull) {
        return "DataBaseUtils.executeQueryValueIfNull(" + Project.getDBManagerName(projectName) + ".getConnection(),"
                + valueIfNull + ",";
    }

    public static SourceFile generateListPage(TableInfo tableInfo, BasicPackages packages, Project project)
            throws Exception {
        ...
        HtmlPageGenerator html = new HtmlPageGenerator(project, "Llista de " + label);
        // Table name
        ...
        html.formCode.append("String __orderFieldStr =" + " request.getParameter(\"orderField\");\n");
        html.formCode
                .append("int __orderField = __orderFieldStr == null ?" + " 0 : Integer.parseInt(__orderFieldStr);\n");
        html.formCode.append("String __orderTypeStr = request.getParameter(\"orderType\");\n");
        html.formCode.append("OrderType __orderType = __orderTypeStr==null ?"
                + " OrderType.ASC : OrderType.valueOf(__orderTypeStr);\n");
        html.formCode.append("String __orderQueryStr = request.getParameter(\"orderQuery\");\n");
        html.formCode.append(
                "boolean __orderQuery = __orderQueryStr == null?" + " false : Boolean.valueOf(__orderQueryStr);\n");
        html.formCode.append("%>\n");
        html.formCode.append("<input type=\"hidden\" name=\"orderField\"" + " value=\"<%=__orderField%>\">\n");
        html.formCode.append("<input type=\"hidden\" name=\"orderType\"" + " value=\"<%=__orderType%>\">\n");
        html.formCode.append("<input type=\"hidden\" name=\"orderQuery\"" + " value=\"<%=__orderQuery%>\">\n");
        ...
        html.tableGenerator = new ListTableGenerator(html.title, project, tableInfo.getNameJava(), fields);
    ...
    public static class ListTableGenerator extends TableGenerator {
         ...
         * @param fields
         */
        public ListTableGenerator(java.lang.String title, Project project, String tableJavaName, FieldInfo[] fields)
                throws Exception {
                ...
                switch (fields[i].webFieldInfo.webtype) {
                    case WebType.Query: {
                        ...
                        tableContent.append("      DataBaseUtils.orderList("
                                + Project.getDBManagerName(project.getProjectName()) + ".getConnection(), list, \""
                                + select + "\", \"" + fields[i].getJavaName() + "\", __orderType);\n");
                        tableContent.append("    };\n");
                    }
                    break;
                    case WebType.UserID:
            ...
            tableContent.append("%>\n");
            tableContent.append("<tr class=\"<%=(x % 2 == 0) ? \"odd\" : \"parell\"%>\">\n");
            String pk = "";
            for (int i = 0; i < fields.length; i++) {
                if (fields[i].isPrimaryKey()) {
                    ...
                    pk = pk + fields[i].getJavaName() + "=<%=URLEncoder.encode(String.valueOf(bean."
            ...
            tableContent.append("  <td><a href=\"" + tableJavaName + "_Delete.jsp?" + pk
                    + "\"><img src=\"./images/delete.gif\" alt=\"Borrar\" /> </a></td>\n");

            for (int i = 0; i < fields.length; i++) {
                tableContent.append("<td>" + getWebMapping(fields[i], ModeWeb.LIST, project) + "</td>\n");
            }
            tableContent.append(" </tr>\n");
            tableContent.append("<% } %>\n");
        }

        public static ConditionalColumn[] generateTitleColumns(String table, FieldInfo[] fields) {
            ...
            for (int j = 0; j < fields.length; j++) {
                ...
                post2 = post + query + "'";
                String fieldID = "<%=" + table + "Fields." + fields[j].javaName.toUpperCase() + ".fieldID%>";
                titles[j + 2] = new ConditionalColumn(
                        ...
                        // XX
                                ...
                                + pre + fieldID + inter + OrderType.ASC + post2 + "\" " + "src=\"./images/up.gif\" >\n"
                                + "<input type=\"image\" onclick=\"" + pre + fieldID + inter + OrderType.DESC + post2
                                + "\" " + "src=\"./images/down.gif\">\n",
                        null);
    ...
    public static String getCalendarField(String id, String value, int webType, boolean isNotNullable) {
        String showsTime;
        String format;
        ...
        return "<table border=0 cellspacing=0 cellpadding=0>\n" + "<tr>\n" + "  <td>\n" + "    <input type=\"text\" "
                + (isNotNullable ? " onfocus=\"checks()\"" : "") + " name=\"" + id + "\" id=\"" + id + "\" value=\""
                + value + "\" readonly />\n" + "</td>\n" + "<td valign=\"middle\">\n"
                + "&nbsp;<img border=\"0\" src=\"calendar/cal.gif\"" + " onclick=\"javascript:NewCssCal" + "('" + id
                + "','" + format + "','arrow'" + showsTime + ")\"" + " style=\"cursor: pointer;\" />\n" + "  </td>\n"
                + "</tr>\n" + "</table>\n";
    }
     ...
     * @return
     */
    public static SourceFile generateEditPage(TableInfo tableInfo, BasicPackages packages, Project project)
            throws Exception {
        String table = tableInfo.getNameJava();
        //String label = tableInfo.getLabel();
        String model = CodeGenUtils.getModelName(tableInfo.getNameJava());
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (fields[i].isPrimaryKey()) {
                ...
                checkPK.append("   error = \"No puc obtenir el par�metre '" + fields[i].javaName + "'\";\n");
                checkPK.append(" }\n");

                String strValue = Converter.toStringValue(fields[i].getJavaType(),
                        "bean." + CodeGenUtils.get(fields[i]), webMode);
                replaceURL.append("        __tmp = __tmp.replace(\"$$PK" + countPK + "$$\"," + strValue + ");\n");
                countPK++;
            }
            requestGetParameter.append("      __bean." + CodeGenUtils.set(fields[i].javaName) + "(" + value + ");\n");
        ...
        String javaCode = "<%\n" + "  " + table + " bean;\n"
                ...
                + "Bean __bean = new " + table + "Bean();\n" + requestGetParameter.toString() + "      bean = __bean;\n"
                ...
                + "    } catch(Exception e) {\n" + "      error = e.getMessage();\n" + "    }\n"
                + "  } // Final If Edit\n" + "  bean = dao.get" + table + "Manager().findByPrimaryKey("
                + createParams.toString() + ");\n" + "  \n" + "%>\n";

        String formCode = "   <% if (redirectUrl != null) { %>\n"
                + "   <input type=\"hidden\" name=\"redirectUrl\" value=\"<%=redirectUrl%>\">\n" + "   <% } %>\n";

        StringBuffer jsp = generateNewOrEditJsp(packages, project, pageTitle, buttonTitle, webMode, tableInfo, javaCode,
                formCode);
     ...
     * @return
     */
    public static SourceFile generateDeletePage(Project project, TableInfo tableInfo, String modelPackage //, String daoPackage
    ) {
     ...
     * @return
     */
    public static SourceFile generateAddPage(TableInfo tableInfo, BasicPackages packages, Project project)
            throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            if (fields[i].isPrimaryKey()) {
                ...
                String value = Converter.toStringValue(fields[i].getJavaType(), "bean." + CodeGenUtils.get(fields[i]),
                        webMode);
                primaryKeys.append(fields[i].javaName + "=\" + URLEncoder.encode(" + value + ")");

                replaceURL.append("        __tmp = __tmp.replace(\"$$PK" + countPK + "$$\"," + value + ");\n");
                countPK++;
            }
            ...
            added = true;
            String value = Converter.toJavaValue(fields[i], "request.getParameter(\"" + fields[i].javaName + "\")");
            createParams.append("bean." + CodeGenUtils.get(fields[i]));
            requestGetParameter.append("bean." + CodeGenUtils.set(fields[i].javaName) + "(" + value + ");\n");
        ...
        String javaCode = "<%\n" + "  " + table + " bean;\n" + "  String error = null;\n"
                ...
                + "Bean();\n" + requestGetParameter.toString()
                + "  if (request.getParameter(\"loadedPage\") != null) {\n" + "    try {\n" + "      bean = dao.get"
                + table + "Manager().create(" + createParams.toString() + ");\n" + "      if (redirectUrl != null) {\n"
                + "        String __tmp = java.net.URLDecoder.decode(redirectUrl);\n" + replaceURL.toString()
                + "        response.sendRedirect(__tmp);\n" + "      } else {" + "          response.sendRedirect(\""
                + table + "_Edit.jsp?" + primaryKeys.toString() + ");\n" + "      }\n" + "    } catch(Exception e) {\n"
                + "      error = e.getMessage();\n" + "    }\n" + "  } // Final If Create\n" + "%>\n";
    ...
    private static StringBuffer generateNewOrEditJsp(BasicPackages packages, Project project, String title,
            String buttonTitle, ModeWeb webMode, TableInfo tableInfo, String javaCode, String formCode)
            throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            values[i][0] =
                    ...
                    // XX
                            + "[ca:" + fields[i].getLabels().get("ca") + "]" + (fields[i].isNotNullable ? " (*)" : "");
        ...
        html.javaScript.append("  document." + HtmlPageGenerator.FORM_NAME + "." + HtmlPageGenerator.SUBMITBUTTON
                + ".disabled = false;\n");
        html.javaScript.append("}\n\n");
    ...
    public static String generateIsEmptyJavascript(String field) {
        ...
        return "  if (document." + FORM_NAME + "." + field + ".value==null ||" + " document." + FORM_NAME + "." + field
        ...
        switch (webType) {
            ...
            case WebType.Query:
                if (field.isNotNullable) {
                    String item = "document." + HtmlPageGenerator.FORM_NAME + "." + field.javaName;
                    return "     var selIndex_" + field.javaName + " = " + item + ".selectedIndex;\n"
                            + "     var value_" + field.javaName + " = " + item + ".options[selIndex_" + field.javaName
                            + "].value;\n" + "     if (value_" + field.javaName + "==null || value_" + field.javaName
                            + ".length==0) {\n" + "       document." + HtmlPageGenerator.FORM_NAME + "."
            ...
            case WebType.Decimal:
                otherChecks = "    check" + webType + "(document." + HtmlPageGenerator.FORM_NAME + "." + field.javaName
                        + ");\n";
            case WebType.URL:
            case WebType.File:
            case WebType.Text:
            ...
            default:
                return "alert(\"Unknow type " + webType + " generating JavaScript.\"); return;\n";
        }

    }

    public static String getWebMapping(FieldInfo field, ModeWeb mode, Project project) throws Exception {

        int webType = field.webFieldInfo.webtype;

        switch (webType) {
            case WebType.Checkbox: {
                return "<INPUT NAME=" + field.getJavaName() + " type=\"checkbox\" " + " <%=bean."
                        + CodeGenUtils.get(field) + "?\"checked\":\"\"%> " + (mode == ModeWeb.LIST ? "DISABLED" : "")
                        + " >\n";
            }
            ...
            case WebType.Time: {
                switch (mode) {
                    case LIST:
                        return "<%="
                                + Converter.toStringValue(field.getJavaType(), "bean." + CodeGenUtils.get(field), mode)
                                + "%>";
                    case EDIT:
                    default:
                    case NEW:
                        return getCalendarField(field.getJavaName(), "<%="
                                + Converter.toStringValue(field.getJavaType(), "bean." + CodeGenUtils.get(field), mode)
                                + "%>", webType, field.isNotNullable);
                }
            }
            ...
            case WebType.PrimaryKey:
                switch (mode) {
                    case LIST:
                    case EDIT:
                        return "<input type=hidden name=" + field.getJavaName() + " value=\"<%=bean."
                                + CodeGenUtils.get(field) + "%>\" ><%=bean." + CodeGenUtils.get(field) + "%>";
                    default:
                    case NEW:
                        if (field.isAutoIncrement()) {
                            return "<b>Autoincrement Field</b>";
                        } else {
                            return "<input type=\"text\" onkeyup=\"checks()\" name=\"" + field.getJavaName()
                                    + "\" value=\"<%=" + Converter.toStringValue(field.getJavaType(),
            ...
            case WebType.Query: {
                ...
                switch (mode) {
                    case LIST:
                        String select = ProjectUtils.createSelect(fk.getTable(), fkField,
                                " where " + fkField.sqlName + " = ?");
                        return ("<%=" + EXECUTEQUERYMETHODNAME(project.getProjectName(), NULL_IMG) + "\"" + select
            ...
            case WebType.TextArea: {
                switch (mode) {
                    case LIST:
                        return "<% if (bean." + CodeGenUtils.get(field) + " == null) { %>\n" + "&nbsp;\n"
                                + "<% } else { %>\n" + "   <textarea readonly style=\"overflow:auto;\""
                                + " rows=\"1\" cols=\"20\"><%=bean." + CodeGenUtils.get(field) + "%></textarea>"
                                + "<% } %>";
                    case EDIT:
                    default:
                    case NEW:
                        return "<textarea rows=\"2\" cols=\"80\" name=\"" + field.getJavaName() + "\"><%=" + Converter
                                .toStringValue(field.getJavaType(), "bean." + CodeGenUtils.get(field) + "", mode)
                                + "%></textarea>";
                }
            }
            ...
            case WebType.RichText: {
                ...
                switch (mode) {
                    case LIST:
                        random = "<%=java.lang.Math.abs(" + "new java.util.Random(System.nanoTime()).nextInt())%>";
                        generatedRichTextJavascript = true;
                        return "<% if (bean." + CodeGenUtils.get(field) + " == null) { %>\n" + "&nbsp;\n"
                                + "<% } else { %>\n" + "   <textarea id=\"" + random
                                + "\" class=\"mceEditor\" rows=\"1\" cols=\"20\"><%=bean." + CodeGenUtils.get(field)
                                + "%></textarea>" + "<% } %>";
                    case EDIT:
                    default:
                    case NEW:
                        ...
                        return "<table class=\"dummy\" style=\"background-color:white;\">\n"
                                ...
                                + random + "\" class=\"mceEditor\"" + " rows=\"2\" cols=\"80\"  name=\"" + name
            ...
            case WebType.File: {
                ...
                switch (mode) {
                    case LIST:
                        ...
                        txt = "<% if (bean." + CodeGenUtils.get(field) + " != null) { %> " + "<a href=\"<%=\""
                                + field.webFieldInfo.getLabel()[0] + "/\" + bean." + CodeGenUtils.get(field)
                                + "%>\"><%=bean." + CodeGenUtils.get(field) + "%></a> <% } %>";
                    break;
                    case EDIT:
                        txt = txt + "<% if (bean." + CodeGenUtils.get(field) + " != null) { %> " + "Current: <%=bean."
                ...
                switch (mode) {
                    case LIST:
                        if (webType == WebType.URL) {
                            return "<% if (bean." + CodeGenUtils.get(field) + " != null) {%>\n" + "<a href=\"<%=bean."
                                    + CodeGenUtils.get(field) + "%>\" target=\"_blank\"><%=bean."
                                    + CodeGenUtils.get(field) + "%></a>\n" + "<% } %>\n";
                        } else {
                            return "<%=" + Converter.toStringValue(field.getJavaType(),
                                    "bean." + CodeGenUtils.get(field) + "", mode) + "%>";
                        }
                    case EDIT:
                    default:
                    case NEW:
                        ...
                        if (field.getSize() != null) {
                            ...
                            sizes = "maxlength=\"" + size + "\" size=\"" + Math.min(80, Integer.parseInt(size)) + "\"";
                        }
                        return "<input type=\"text\" name=\"" + field.getJavaName() + "\" " + sizes
                                + " onkeyup=\"checks()\" \n" + " value=\"<%=" + Converter.toStringValue(
                                        field.getJavaType(), "bean." + CodeGenUtils.get(field) + "", mode)
                                + "%>\" >";
                }
            }
        }
    ...
    private static String generateSelectHtml(Project project, FieldInfo field, ForeignKey fk, FieldInfo fkField)
            throws Exception {

        StringBuffer values = new StringBuffer("<%\n");
        values.append("Object _obj_;\n");
        values.append("StringBuffer values_" + field.getJavaName() + " = new StringBuffer();\n");
        ...
        if (labels != null) {
            for (int i = 0; i < labels.length; i++) {
                ...
                values.append(
                        "_obj_ = items_" + field.getJavaName() + "[i]." + CodeGenUtils.get(labels[i].trim()) + ";\n");
                values.append("if(_obj_ != null) {\n");
                values.append("  values_" + field.getJavaName() + ".append(_obj_);\n");
                values.append("}\n");
                order.append(getJavaTableName(project, fk.getTable()) + "Fields." + labels[i].trim().toUpperCase());
            }
        }
        values.append("%>\n");

        String selectCode = "<% " + field.getJavaType().getName() + " selected_" + field.getJavaName() + " = bean."
                + CodeGenUtils.get(field) + "; %>\n" + "<select "
                + (field.isNotNullable ? " onchange=\"checks()\"" : "") + " name=\"" + field.getJavaName() + "\">\n"
                ...
                + "    for (int i = 0; i < items_" + field.getJavaName() + ".length; i++) { %>" + values.toString()
                + "   <option value=\"<%=items_" + field.getJavaName() + "[i]." + CodeGenUtils.get(fk.getField().trim())
                + "%>\" <%=" + GenAppPackages.PKG_BASE + ".common.RebUtils.equals(items_" + field.getJavaName() + "[i]."
                + CodeGenUtils.get(fk.getField().trim()) + ",selected_" + field.getJavaName()
                + ")?\"selected\":\"\"%> >" + "<%= values_" + field.getJavaName() + ".length() < 51 ? values_"
    ...
    public static String getJspBaseName(Project project) {
        return project.getPackageName() + "." + WebGenerator.WEB_PACKAGE_NAME + "." + project.getProjectName()
                + "JspBase";
    }
    ...
    public static SourceFile generateJspBase(Project project, String packageServlet) {
        ...
        String javaCode = "package " + packageServlet + ";\n" + "/*\n" + "  YOU CAN MODIFY THIS CLASS\n"
                + "  IF GENERATOR SEE THAT YOU HAS MODIFIED THIS CLASS" + " THEN DO NOT OVERWRITE IT. Thanks.\n" + "\n"
                + "*/\n" + "public abstract class " + project.getProjectName() + "JspBase \n" + "  extends "
                + packageServlet + "." + project.getProjectName() + "AbstractJspBase {\n" + "\n" + "}\n";
        return new SourceFile(project.getProjectName() + "JspBase.java", javaCode);
    }

    public static SourceFile generateAbstractJspBase(Project project, String webPackage, String modelPackage) {

        String managerSec = project.getPackageName() + ".security."
                + Project.getSecManagerName(project.getProjectName());

        String javaCode = "package " + webPackage + ";" + "\n" + "import javax.servlet.ServletException;\n"
                + "import javax.servlet.ServletConfig;\n" + "import " + modelPackage + ".*;\n" + "import "
                + GenAppPackages.PKG_BASE + ".common.security.ISecurity;" + "\n" + "import " + GenAppPackages.PKG_BASE
                + ".common.REB2010JspBase;" + "\n" + "/*" + "\n" + "  PLEASE DO NOT OVERWRITE THIS FILE !!!!!" + "\n"
                + "  PLEASE MODIFY THE CLASS " + webPackage + "." + project.getProjectName() + "JspBase. Thanks." + "\n"
                + "   " + "\n" + "*/" + "\n" + "public abstract class " + project.getProjectName()
                + "AbstractJspBase extends REB2010JspBase {" + "\n" + "\n" + "  protected ISecurity security;\n"
                + "  protected I" + project.getProjectName() + "DaoManagers dao;\n\n" + "  /** \n"
                + "  * @param config\n" + "  * @throws ServletException\n" + "  */\n"
                + "  public void init(ServletConfig config) throws ServletException {\n" + "    super.init(config);\n"
                + "    try {\n" + "      dao = " + project.getProjectName() + "DaoManager.getDaoManagers();\n"
                + "      security = " + managerSec + ".getSecurity();\n" + "    } catch (Exception e) {\n"
                + "      e.printStackTrace();\n" + "      throw new ServletException(e);\n" + "    }\n" + "  }\n\n"

                + "  public ISecurity getSecurity() {\n" + "    return security;\n" + "  };\n\n" + "}\n";

        return new SourceFile(project.getProjectName() + "AbstractJspBase.java", javaCode);
    ...
    static class Converter {
        public static String toJavaValue(FieldInfo field, String code) {
            ...
            if (javaType.isPrimitive()) {
                ...
                String base = "( (" + code + " == null) || (" + code + ".trim().length() == 0) ) ? ";
            ...
            if (javaType.equals(Date.class)) {
                return "(" + code + " == null || " + code + ".trim().equals(\"\") )? null : new java.sql.Date("
                        + "getSimpleDateFormat().parse(" + code + ").getTime())";
            }

            if (javaType.equals(Timestamp.class)) {
                return "(" + code + " == null || " + code + ".trim().equals(\"\") )? null : new java.sql.Timestamp("
                        + "getSimpleDateTimeFormat().parse(" + code + ").getTime())";
            }

            if (javaType.equals(Time.class)) {
                return "(" + code + " == null || " + code + ".trim().equals(\"\"))? null : new java.sql.Time("
                        + "getSimpleTimeFormat().parse(" + code + ").getTime())";
            }
            ...
            if (javaType.equals(String.class)) {
                if (field.isNotNullable) {
                    ...
                    // Pot ser null
                    return "( (" + code + " == null) || (" + code + ".equals(\"\") ) ? null :" + code + ")";
                }
            }

            if (javaType.equals(java.math.BigDecimal.class) || javaType.equals(java.math.BigInteger.class)
                    || javaType.equals(Byte.class) || javaType.equals(Short.class) || javaType.equals(Integer.class)
                    || javaType.equals(Long.class) || javaType.equals(Double.class) || javaType.equals(Float.class)) {
                return "(" + code + " == null || " + code + ".trim().equals(\"\") )? null : new " + javaType.getName()
                        + "(" + code + ")";
            }

            return "UNKNOWN TYPE " + javaType + " TO CONVERT FROM STRING(" + code + ") TO JAVA";
        }

        public static String toStringValue(Class<?> javaType, String code, ModeWeb mode) {
            ...
            if (javaType.equals(java.sql.Date.class)) {
                return "(" + code + "== null)? " + ((ModeWeb.LIST == mode) ? NULL_IMG : "\"\"")
                        + " : getSimpleDateFormat()." + "format(new java.util.Date(" + code + ".getTime()))";
            }

            if (javaType.equals(java.sql.Timestamp.class)) {
                return "(" + code + "== null)? " + ((ModeWeb.LIST == mode) ? NULL_IMG : "\"\"")
                        + " : getSimpleDateTimeFormat()." + "format(new java.util.Date(" + code + ".getTime()))";
            }

            if (javaType.equals(java.sql.Time.class)) {
                return "(" + code + "== null)? " + ((ModeWeb.LIST == mode) ? NULL_IMG : "\"\"")
                        + " :getSimpleTimeFormat().format(new java.util.Date(" + code + ".getTime()))";
            }
            ...
            if (javaType.equals(java.math.BigDecimal.class) || javaType.equals(Double.class)
                    || javaType.equals(Float.class)) {
                return "(" + code + "==null)? " + ((ModeWeb.LIST == mode) ? NULL_IMG : "\"\"") + " : " + code
                        + ".toString()";
            }

            if (javaType.equals(java.math.BigInteger.class) || javaType.equals(java.math.BigDecimal.class)
                    || javaType.equals(Byte.class) || javaType.equals(Short.class) || javaType.equals(Long.class)
                    || javaType.equals(Integer.class) || javaType.equals(Double.class)
                    || javaType.equals(Float.class)) {
                return "(" + code + "==null)? " + ((ModeWeb.LIST == mode) ? NULL_IMG : "\"\"") + " : " + code
                        + ".toString()";
            }

            return "UNKNOWN TYPE " + javaType + " TO CONVERT FROM JAVA(" + code + ") TO STRING";
        }
