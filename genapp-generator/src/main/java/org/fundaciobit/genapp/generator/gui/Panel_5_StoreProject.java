...
public class Panel_5_StoreProject extends Paneable {
            ...
            if (errorgreu) {
                ...
                if (multipleUniques != null /* && multipleUniques.length == 1 */) {
                    ...
                    for (int i = 0; i < multipleUniques.length; i++) {
...

                        {
                            ...
                            String ukNameStartsExpected = SharedData.data.getPrefix() + "_" + shortStr + "_";
                ...
                for (FieldInfo field : table.getFields()) {
                    ...
                    for (ForeignKey fk : fkImport) {
                        ...
                        String fkNameStartsExpected = SharedData.data.getPrefix() + "_" + shortStr + "_"
                    ...
                    if (index == null) {
                        ...
                        if (fkImport != null && fkImport.length != 0) {
                            ...
                            System.err.println(avis + " create index " + expected + " on " + table.name + " ("
                                    + field.sqlName + ");\n");
                        ...
                        if (field.isPrimaryKey()) {
                            ...
                            if ((index.startsWith(table.name + "_")
                                    || index.startsWith(SharedData.data.getPrefix() + "_" + shortStr + "_"))
                                    && index.endsWith("_fk_i") && index.length() <= 30) {
                                // OK
                            } else {
                                String expected = getExpectedIndexFK(table, shortStr, field);
                                System.err.println(" --  Error de forma en l'index FK  " + index
                            ...
                            if ((index.startsWith(table.name + "_")
                                ...
                                if (expected.length() > 30) {
                                    expected = SharedData.data.getPrefix() + "_" + shortStr + "_" + field.sqlName
                                            + "_i";
                                }
                                String avis = (expected.length() > 30) ? "**" : "";
    ...
    private String checkExpectedIndexPK(TableInfo table, String shortStr, FieldInfo field, int countPKs, String index) {
        ...
        if (index == null) {
            ...
            System.err.println(" create index " + expected + ((expected.length() > 30) ? "**" : "") + " on "
    ...
    private void checkTraduccions(String[] idiomes, String name, Map<String, String> labelsTaula, StringBuffer errors) {
        for (String lang : idiomes) {
