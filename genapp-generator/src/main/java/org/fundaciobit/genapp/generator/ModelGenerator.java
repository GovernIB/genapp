...
public class ModelGenerator {
    ...
    public static SourceFile generateInterfacesManager(BasicPackages packages, Project project) {
        ...
        String name = "I" + project.getProjectName() + "DaoManagers";
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            String managerFileName = tables[i].getNameJava() + "Manager";
    ...
    public static SourceFile generateManagerOfDaoManagers(String modelPackage, String project) {
        ...
        manager.append("      throw new Exception(\"Ha de inicialitzar el sistema de Managers cridant \"\n");
        manager.append("          + \" al mètode " + project + "DaoManager.setDaoManagers(...)\");\n");
        manager.append("    }\n");
    ...
    public static SourceFile generateInterfacesManager(TableInfo tableInfo, BasicPackages packages, FieldInfo[] fields,
            BeanCode beanCode) {
    ...
    public static SourceFile generateTableFields(String tableNameSQL, BasicPackages packages, String javaName,
            FieldInfo[] fields, BeanCode beanCode) {
        ...
        // interfaceCode.append("\n\n");
        interfaceCode.append("public interface " + codeFileName + " extends java.io.Serializable {");
        interfaceCode.append("\n\n");
        interfaceCode.append("  public static final String _TABLE_SQL = \"" + tableNameSQL + "\";\n");
        interfaceCode.append("\n\n");

        interfaceCode.append(
                "  public static final String _TABLE_MODEL = \"" + CodeGenUtils.getModelName(javaName) + "\";\n");
        interfaceCode.append("\n\n");
        ...
        interfaceCode.append("  public static final Field<?>[] ALL_" + javaName.toUpperCase() + "_FIELDS = {\n");
    ...
    public static SourceFile generateInterfaceBase(Project project, String packageName, TableInfo table,
        ...
        String codeImportsExports = DaoJPAGenerator.generateImportsExports(project, table, fields, imports, null,
                CodeType.INTERFACE);
        ...
        /*
         * String fileInterface = ""; if (table.isFileMappingEntity() ) { fileInterface
         * = ",\n     " + GenAppPackages.PKG_BASE + ".common.filesystem.IFile "; }
         * String idiomaInterface = ""; if (table.isLanguageMappingEntity()) {
         * idiomaInterface = ",\n   " + GenAppPackages.PKG_BASE +
         * ".common.language.ILanguage "; }
         */
        ...
        if (fileTable != null) {
            ...
            for (FieldInfo field : fileFields) {
                ...
                interfaceCode.append(
                        "  public <F extends " + fileTableName + "> F " + CodeGenUtils.get(javaNameField) + ";\n");
    ...
    public static SourceFile generatePKClass(String tableNameJava, BasicPackages packages, BeanCode beanCode) {
    ...
    public static SourceFile generateManagerBD(Project project, String packageName) {
        ...
        String manager = Project.getDBManagerName(project.getProjectName());
        ...
        dbman.append(" * Change values of this class using " + project.getProjectName() + "Initializations.java class.\n");
        ...
        dbman.append("       throw new Exception(\"" + "You must define database info using " + manager
                + ".setDataBase(DataBaseInfo DataBaseInfo) method.\");\n");
        dbman.append("    }\n");
        ...
        dbman.append("\n");
        dbman.append(
                "  public static DataBaseInfo getDataBaseInfoFromFile(File f)" + " throws FileNotFoundException {\n");
    ...
    public static SourceFile generateBeanCode(Project project, TableInfo table, BasicPackages packages,
            BeanCode beanGenCode) throws Exception {
        ...
        beanCode.append(
                generateCopyBeanCode(table, fields, codeBeanFileName, "toBean", fileFields, fileTable, "Bean", true));
        ...
        beanCode.append(
                generateCopyBeanCode(table, fields, codeBeanFileName, "toBean", fileFields, fileTable, "Bean", false));
        ...
        if (table.isFileMappingEntity()) {
            ...
            if (fileTable != null) {
                ...
                for (FieldInfo field : fileFields) {
                    ...
                    // Mètode set
                    beanCode.append(
                            "  public void " + CodeGenUtils.set(javaNameField) + "(" + fileTableName + " __field) {\n");
    ...
    public static String generateCopyBeanCode(TableInfo table, FieldInfo[] fields, String codeBeanFileName,
            String methodName, List<FieldInfo> fileFields, TableInfo fileTable, String fileSuffix,
        ...
        if (isConstructor) {
            ...
            beanCode.append(
                    "  public static " + codeBeanFileName + " " + methodName + "(" + tableNameJava + " __bean) {\n");
        ...
        for (int r = 0; r < fields.length; r++) {
            beanCode.append("    " + var + "." + CodeGenUtils.set(fields[r].javaName) + "(" + "__bean."
                    + CodeGenUtils.get(fields[r]) + ");\n");
        }
        if (fileTable != null && tableNameJava.equals(fileTable.getNameJava())) {
            ...
            if (fileFields != null) {
                for (FieldInfo field : fileFields) {
                    String javaNameField = field.getJavaName();
                    if (!javaNameField.endsWith("ID")) {
                        throw new Exception("El camp " + field.getJavaName() + " de la taula " + tableNameJava
                                + " fa referencia a la taula externa [FITXERS BBDD]"
                                + " però aquest camp java no acaba amb ID");
                    }
                    javaNameField = javaNameField.substring(0, javaNameField.length() - 2);
