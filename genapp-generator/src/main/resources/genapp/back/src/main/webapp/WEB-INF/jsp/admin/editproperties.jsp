...
<c:if test="${symbol_dollar}{not empty success}">
    <div class="msg success">${symbol_dollar}{success}</div>
</c:if>
<c:if test="${symbol_dollar}{not empty error}">
    <div class="msg error">${symbol_dollar}{error}</div>
</c:if>

<form method="post" action="${symbol_dollar}{pageContext.request.contextPath}/admin/editproperties">
<div style="resize: both; overflow: auto; border:1px solid #ccc; margin:20px;height:60vh;">
    <textarea id="editor" name="fileContent" >${symbol_dollar}{fileContent}</textarea>
</div>
    <br />
