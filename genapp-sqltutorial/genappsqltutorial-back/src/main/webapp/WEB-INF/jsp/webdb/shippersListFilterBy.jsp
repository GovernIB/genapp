...
<un:useConstants var="ShippersFields" className="org.fundaciobit.genappsqltutorial.model.fields.ShippersFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,ShippersFields.SHIPPERID)}">
            <%-- FILTRE NUMERO --%>      
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
              <span class="add-on"><fmt:message key="shippers.shipperid" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,ShippersFields.SHIPPERNAME)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="shippers.shippername" var="shippername" />
              <fmt:message key="genapp.form.searchby" var="cercapershippername" >                
                 <fmt:param value="${shippername}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${shippername}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercapershippername}" path="shippername" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,ShippersFields.PHONE)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="shippers.phone" var="phone" />
