...
<un:useConstants var="OrderDetailsFields" className="org.fundaciobit.genappsqltutorial.model.fields.OrderDetailsFields"/>
        ...
        <c:if test="${ __entry.key < 0  && ((empty __entry.value.searchBy)? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.searchBy)) && ((empty __entry.value.groupBy )? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.groupBy ))}">
          <td>
             <c:if test="${not empty __entry.value.valueMap }">
               <c:out escapeXml="${__entry.value.escapeXml}" value="${__entry.value.valueMap[orderDetails.orderdetailid]}" />
             </c:if>
        ...
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,OrderDetailsFields.ORDERID)}">
          <td>
          <c:set var="tmp">${orderDetails.orderid}</c:set>
          <c:if test="${not empty tmp}">
          ${__theFilterForm.mapOfOrdersForOrderid[tmp]}
          </c:if>
          </td>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,OrderDetailsFields.PRODUCTID)}">
          <td>
          <c:set var="tmp">${orderDetails.productid}</c:set>
          <c:if test="${not empty tmp}">
          ${__theFilterForm.mapOfProductsForProductid[tmp]}
          </c:if>
          </td>
        </c:if>
