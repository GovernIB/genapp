...
public class LogicForBaseFilterForm {
    ...
    private static final Where EMPTY_WHERE = new Where() {

        @Override
        protected int setValues(Query query, int index) throws I18NException {
            return index;
        }

        @Override
        public QuerySQL toSQL(int index) {
            return new QuerySQL(index, "0=0");
        }
                    ...
                    if ((_valueFrom != null && _valueFrom.trim().length() != 0)
                        ...
                        try {
                            if ((searchBy instanceof LongField)) {
                                ...
                                __wheres.add(
                                        getWhere(BigInteger.class, (Field<Number>) searchBy, _valueFrom, _valueTo));
                            } else if (searchBy instanceof BigDecimalField) {
                                __wheres.add(
                                        getWhere(BigDecimal.class, (Field<Number>) searchBy, _valueFrom, _valueTo));
                            } else {
                            ...
                            log.error(
                                    "Error intentant filtrar pel camp " + searchBy.fullName + " amb el valors [FROM: {"
                                            + _valueFrom + "} | TO: {" + _valueTo + "}]: " + e.getMessage(),
                                    e);
        ...
        if (filterForm.getFilterByFields() != null) {
            for (Field<?> f : filterForm.getFilterByFields()) {
                ...
                if (f instanceof DateField) {
                    ...
                    java.sql.Date _valueDesde = (java.sql.Date) Utils.getValueOfJavaField(this.filterForm,
                            f.javaName + "Desde");
                    if (_valueDesde != null) {
                        __wheres.add(df.greaterThanOrEqual(_valueDesde));
                    }

                    java.sql.Date _valueFins = (java.sql.Date) Utils.getValueOfJavaField(this.filterForm,
                            f.javaName + "Fins");
                    if (_valueFins != null) {
                ...
                // NUMBER 
                if ((f instanceof LongField) || (f instanceof IntegerField) || (f instanceof ByteField)
                        || (f instanceof FloatField) || (f instanceof DoubleField) || (f instanceof ShortField)
                    ...
                    if (_values != null) {
                        ...
                        Number _valueDesde = (Number) Utils.getValueOfJavaField(this.filterForm, f.javaName + "Desde",
                                false);
                        if (_valueDesde != null) {
                            __wheres.add(ff.greaterThanOrEqual(_valueDesde));
                        }

                        Number _valueFins = (Number) Utils.getValueOfJavaField(this.filterForm, f.javaName + "Fins",
                                false);
                        if (_valueFins != null) {
                ...
                log.error("No s'ha trobat gestor en el metode getFilterWhere de la classe " + this.getClass()
    ...
    public static FilterFormData getFilterFormData(BaseFilterForm filterForm, ITableManager<?, ?> ejb,
    ...
    public final FilterFormData getFilterFormData(ITableManager<?, ?> atm, Where whereAdditionalCondition)
        ...
        Where gw = getGroupWhere();
        
        if (gw == null) {
            filterForm.setVisibleGroupBy(false);
        } else if (gw.equals(EMPTY_WHERE)) {
            // Es d'una taula virtual que necessitam mostrar groupBy pero no executar Where
            gw = null;
    ...
    public class FilterSearchGeneratorOfNumber<T extends Number> {
        ...
        public FilterSearchGeneratorOfNumber(Class<T> clazz, Field<Number> searchBy, String from, String to) {
            this.clazz = clazz;
            this.from = from;
            this.to = to;
     ...
     * @throws Exception
     */
    protected void loadDataForGroupByFilterBy(ITableManager<?, ?> atm, Map<String, Object> mav,
            Where whereAdditionalCondition) throws I18NException {
        ...
        //if (filterForm.getGroupByFields() != null) 
        {
            ...
            for (Field<?> f : filterForm.getGroupByFields()) {
                ...
                if (f instanceof TimestampField) {
                    groupByItems.addAll(
                            loadDataForGroupByDateAndTimestamp((TimestampField) f, atm, whereAdditionalCondition));
                    continue;
                }
                if (f instanceof TimeField) {
                ...
                if (f instanceof BooleanField || f instanceof StringField || f instanceof IntegerField
                        || f instanceof LongField || f instanceof DoubleField || f instanceof FloatField
    ...
    private Collection<GroupByItem> loadDataForGroupByDateAndTimestamp(Field<? extends Date> field,
     ...
     * @throws Exception
     */
    private GroupByItem loadDataForGroupByTime(Field<? extends Date> field, ITableManager<?, ?> atm,
            Where whereAdditionalCondition) throws I18NException {
     ...
     * @return
     */
    public GroupByItem checkSelected(Field<?> field, List<GroupByValueItem> gbi) {
     ...
     * @throws Exception
     */
    private GroupByItem loadDataForGroupByField(Field<?> field, ITableManager<?, ?> atm, Where whereAdditionalCondition)
            throws I18NException {
    ...
    public static void addGroupByValuesToVirtualField(BaseFilterForm filterForm, ModelAndView mav,
            Map<Field<?>, GroupByItem> groupByItemsMap, Map<StringKeyValue, Long> _listSKV, Field<?> field) {
        List<GroupByValueItem> values = new ArrayList<>();

        for (Map.Entry<StringKeyValue, Long> skv : _listSKV.entrySet()) {
            String codeLabel = skv.getKey().getValue();
            String value = skv.getKey().getKey();
            // Long count
            values.add(new GroupByValueItem(field, codeLabel, value, false, skv.getValue()));
        }

        GroupByItem gbi = new LogicForBaseFilterForm(filterForm).checkSelected(field, values);
        groupByItemsMap.put(field, gbi);

        List<GroupByItem> groupByItems = (List<GroupByItem>) mav.getModel().get("groupby_items");
